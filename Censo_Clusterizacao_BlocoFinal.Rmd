...
```{r OCUPADOS_DIF_AP, echo=FALSE, message=FALSE, warning=FALSE}
palete4 <- c("#b2182b", "#ef8a62", "#fddbc7", "#c7eae5", "#80cdc1", "#01665e")

SHP_mapasfinais %>%
  mutate(Dif_OCU_NAF = Dif_OCU_NAF * 100) %>%
  filter(!is.na(LOCALIZACAO)) %>%
  filter(!is.na(Dif_OCU_NAF)) %>%
  tm_shape() +
  tm_fill(col = "Dif_OCU_NAF", 
          title = "Diferença percentual de ocupados<br>nos estabelecimentos Não-AF (2006/2017)", 
          id = "LOCALIZACAO", palette=palete4,
          #n=7
          legend.format=list(fun=function(x) paste0(formatC(x, digits=0, big.mark = " ", format="f"), "%")),
          style = "fixed",
...
```{r MAPAS_DifPercent_RENDA_AP, echo=FALSE, message=FALSE, warning=FALSE}
### Diferença percentual de rendimentos totais dos estabelecimentos 
...
#Total
SHP_mapasfinais %>%
  mutate(Dif_REND_OCU_NAF = Dif_REND_OCU_NAF * 100) %>%
  filter(!is.na(LOCALIZACAO)) %>%
  filter(!is.na(Dif_REND_OCU_NAF)) %>%
  tm_shape() +
  tm_fill(col = "Dif_REND_OCU_NAF", 
          title = "Alteração percentual nos<br>rendimentos por ocupados nos<br>estab. da Não-AF de 2006* para 2017", 
          id = "LOCALIZACAO", palette=palete3,
          #n=7
          legend.format=list(fun=function(x) paste0(formatC(x, digits=0, big.mark = ".", format="f"), "%")),
          style = "fixed", 
          breaks = c(-90, -50, -5, 5, 100, 500, 1200)
          ) +
  tm_borders(lwd = 0.2) +
  #tm_facets(by = "TIPOLOGIA", nrow = 1, sync = TRUE, free.coords = TRUE) +
  tm_view(view.legend.position = c("right", "bottom")) +
...
X6853_Organico <- read_excel("data/tabelas/micro_meso_uf_macro_brasil/communicipio/6853_Organico.xlsx", na = "X") 
X6853_Organico$COD[X6853_Organico$NIVEL == "BR"] <- 000
X6853_Organico <- X6853_Organico %>% #deletando colunas pra que elas não fiquem duplicadas quando for juntar as tabelas
  select(., -NIVEL) %>% 
  select(., -LOCALIZACAO)
...
todastabelas_extra <- X6962_Internet %>% 
  ...
  left_join(., X6853_Organico, by = "COD") %>%
...
microrregioes_extra_pct <- microrregioes_extra %>%
  mutate(pct_INTERNET_sim = INTERNET_sim/INTERNET_tot ,
         ...
         pct_Organico_sim_AF  = Organico_sim_AF / Organico_AF ,
         pct_Organico_sim_MP  = Organico_sim_MP / Organico_MP ,
         pct_Organico_sim_NAF  = Organico_sim_NAF / Organico_NAF 
...
EnerEle_nao_NAF <-  SHP_bloco3 %>%
  ...
  tm_fill(col = "pct_EnerEle_nao_NAF", 
          title = "% sem energia elétrica (Não-AF)", 
          id = "nome", 
          palette=c("#5e4fa2", "#66c2a5", "#e6f598", "#fee08b", "#f46d43","#9e0142"),
          n=6,
          legend.format=list(text.separator = 'a', fun=function(x) paste0(round(x*100), "%")),
...
```{r ensinomedio, echo=FALSE, message=FALSE, warning=FALSE, out.width='100%'}
### ENSINO MÉDIO ####
EM_AF <-  SHP_bloco3 %>%
...
```{r ensinosuperioroumais, echo=FALSE, message=FALSE, warning=FALSE, out.width='100%'}
### ENSINO superior ####
SUP_AF <-  SHP_bloco3 %>%
...
```{r organicos, echo=FALSE, message=FALSE, warning=FALSE, out.width='100%'}
Organico_sim_AF <-  SHP_bloco3 %>%
  filter(!is.na(pct_Organico_sim_AF)) %>%
  tm_shape(bbox = bbox_new) +
  tm_fill(col = "pct_Organico_sim_AF", 
          title = "AF", 
          id = "nome", 
          palette=paletaspectral_invert,
          n=6,
          legend.format=list(text.separator = 'a', fun=function(x) paste0(round(x*100), "%")),
          style = "fixed",
        breaks = c(0, 0.01, 0.03, 0.07, 0.15, 0.25, 0.36)) +
  tm_shape(SHP_Estados) +
  tm_borders(lwd = 1, col = "black") +
  tm_layout(main.title = "Uso de agricultura ou\npecuária orgânica (AF)", main.title.position = "center", main.title.size = 0.8,
          frame = FALSE, 
          #legend.frame="white",
          legend.title.color="black",
          legend.title.size = 1,
  ...
  tm_scale_bar(position=c("right", "bottom"), breaks = c(0,500,1000)) 
###
Organico_sim_MP <-  SHP_bloco3 %>%
  filter(!is.na(pct_Organico_sim_MP)) %>%
  tm_shape(bbox = bbox_new) +
  tm_fill(col = "pct_Organico_sim_MP", 
          title = "Pronamp", 
          id = "nome", 
          palette=paletaspectral_invert,
          n=6,
  ...
  tm_shape(SHP_Estados) +
  tm_borders(lwd = 1, col = "black") +
  tm_layout(main.title = "Uso de agricultura ou\npecuária orgânica (MP)", main.title.position = "center", main.title.size = 0.8,
          frame = FALSE, 
          #legend.frame="white",
          legend.title.color="black",
          legend.title.size = 1,
  ...
  tm_scale_bar(position=c("right", "bottom"), breaks = c(0,500,1000))
####
Organico_sim_NAF <-  SHP_bloco3 %>%
  filter(!is.na(pct_Organico_sim_NAF)) %>%
  tm_shape() +
  tm_fill(col = "pct_Organico_sim_NAF", 
          title = "Não familiar", 
          id = "nome", 
          palette=paletaspectral_invert,
          n=6,
  ...
  tm_shape(SHP_Estados) +
  tm_borders(lwd = 1, col = "black") +
  tm_layout(main.title = "Uso de agricultura ou pecuária\norgânica (não familiar)", main.title.position = "center", main.title.size = 0.8,
          frame = FALSE, 
          #legend.frame="white",
          legend.title.color="black",
          legend.title.size = 1,
...
tmap_arrange(Organico_sim_AF, Organico_sim_MP, Organico_sim_NAF, 
             sync = TRUE, 
             nrow = 1)
