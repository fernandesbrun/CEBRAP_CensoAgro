...
CADUNICO_aggmicrorTODOS <- CADUNICO_aggmunicipios %>%
  ...
  summarise(
    ...
    renda_media_percapita_2012 = mean(renda_media_percapita_2012, na.rm = TRUE),
    renda_media_percapitaAF_2012 = mean(renda_media_percapitaAF_2012, na.rm = TRUE),
    renda_media_percapita_2015 = mean(renda_media_percapita_2015 , na.rm = TRUE),
    renda_media_percapitaAF_2015  = mean(renda_media_percapitaAF_2015, na.rm = TRUE),
...
CADUNICO_aggmicror <- CADUNICO_aggmunicipios %>%
  ...
  summarise(renda_media_percapita_2012 = mean(renda_media_percapita_2012, na.rm = TRUE),
    renda_media_percapitaAF_2012 = mean(renda_media_percapitaAF_2012, na.rm = TRUE),
    renda_media_percapita_2015 = mean(renda_media_percapita_2015 , na.rm = TRUE),
    renda_media_percapitaAF_2015  = mean(renda_media_percapitaAF_2015, na.rm = TRUE),
    renda_media_percapita_2018 = mean(renda_media_percapita_2018, na.rm = TRUE),
...
CADUNICO_aggmicror <- CADUNICO_aggmicror %>%
  mutate(
    ...
    pct_qtde_familias_2012	=	(	qtde_familias_2012)	/	POP10_Rur,
    pct_qtde_familiasAF_2012	=	(	qtde_familiasAF_2012)	/	POP10_Rur,
    
    pct_qtde_familias_2015	=	(	qtde_familias_2015)	/	POP10_Rur,
    pct_qtde_familiasAF_2015	=	(	qtde_familiasAF_2015)	/	POP10_Rur,
    
    pct_qtde_familias_2018	=	(	qtde_familias_2018)	/	POP10_Rur,
    pct_qtde_familiasAF_2018	=	(	qtde_familiasAF_2018)	/	POP10_Rur,
...
SHP_CADUNICO %>%
  filter(!is.na(pct_qtde_pessoas_2018)) %>%
  filter(pct_qtde_pessoas_2018 <= 1) %>%
  tm_shape() +
  tm_fill(col = "pct_qtde_pessoas_2018", 
          palette = cores_spectral,
          title = "% de residentes na zona<br>rural que recebem BF (2018)",
          id = "LOCALIZACAO",
          n=5,
...
Para a análise da evolução da pobreza rural, ou seja, da variação da porcentagem de pessoas abaixo da linha da pobreza, foi feita a comparação entre os anos de 2012, 2015 e 2018, mas considerando todos sobre uma mesma base populacional (população rural de 2010, de acordo com o Censo Demográfico), uma vez que não foram encontrados dados anuais de contagem de população rural por microrregiões do Brasil. 
Portanto, enfatiza-se que a comparação **não** leva em consideração possíveis mudanças demográficas nas áreas rurais após 2010.


```{r cMAPAS_CADUNICO2_evolu, echo=FALSE, message=FALSE, warning=FALSE}
cores_spectral <- c("#2b83ba", "#abdda4", "#ffffbf", "#fdae61", "#d7191c") 

COMP_CADUNICO_12a15 <- SHP_CADUNICO %>%
  filter(!is.na(dif_qtde_pessoas_2012a2015)) %>%
  filter(marc_pbf == 1) %>%
  tm_shape() +
  tm_fill(col = "dif_qtde_pessoas_2012a2015", 
          palette = cores_spectral,
          title = "Evolução do número de<br>pessoas que recebem<br>o BF (2012 a 2015)",
          id = "LOCALIZACAO",
          #n=5,
          legend.format=list(fun=function(x) paste0(round(x*100), "%")),
          style = "fixed",
          breaks = c(-0.45, -0.1, 0, 0.1, 0.45, 1)
          ) +
  tm_borders(lwd = 0.2) +
  #tm_facets(by = "TIPOLOGIA", nrow = 1, sync = TRUE, free.coords = TRUE) +
  tm_view(view.legend.position = c("right", "bottom")) +
...
COMP_CADUNICO_15a18 <- SHP_CADUNICO %>%
  filter(!is.na(dif_qtde_pessoas_2015a2018)) %>%
  filter(marc_pbf == 1) %>%
  tm_shape() +
  tm_fill(col = "dif_qtde_pessoas_2015a2018", 
          palette = cores_spectral,
          title = "Evolução do número de<br>pessoas que recebem<br>o BF (2015 a 2018)",
          id = "LOCALIZACAO",
          #n=5,
          legend.format=list(fun=function(x) paste0(round(x*100), "%")),
          style = "fixed",
...
tmap_arrange(COMP_CADUNICO_12a15, COMP_CADUNICO_15a18, 
             sync = TRUE, 
             nrow = 1)
