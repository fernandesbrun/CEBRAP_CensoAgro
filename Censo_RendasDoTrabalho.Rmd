...
O presente documento tem como objetivo comparar as rendas obtidas pelo trabalho da agricultura familiar e da não familiar, com a média e mediana das rendas obtidas pelo conjunto total dos estabelecimentos agropecuários no país.

A variável utilizada tem como origem o Censo Agropecuário de 2017, de nome "valor das receitas ou rendas obtidas pelos estabelecimentos agropecuários (Mil Reais)".
...
- Receitas/rendimentos do trabalho obtidos pelos estabelecimentos da AF na unidade espacial possuem valor abaixo da mediana dos rendimentos do trabalho obtidos por todos os estabelecimentos do país;
- Receitas/rendimentos do trabalho obtidos pelos estabelecimentos da AF na unidade espacial possuem valor entre a mediana e a média dos rendimentos do trabalho obtidos por todos os estabelecimentos do país;
- Receitas/rendimentos do trabalho obtidos pelos estabelecimentos da AF na unidade espacial possuem valor acima da média dos rendimentos do trabalho obtidos por todos os estabelecimentos do país 


**Para a agricultura não familiar**
A unidade espacial analisada (municípios ou microrregiões) foi classificada dentre três categorias possíveis:

- Receitas/rendimentos do trabalho obtidos pelos estabelecimentos da não-AF na unidade espacial possuem valor abaixo da mediana dos rendimentos do trabalho obtidos por todos os estabelecimentos do país; 
- Receitas/rendimentos do trabalho obtidos pelos estabelecimentos da não-AF na unidade espacial possuem valor entre a mediana e a média dos rendimentos do trabalho obtidos por todos os estabelecimentos do país; 
- Receitas/rendimentos do trabalho obtidos pelos estabelecimentos da não-AF na unidade espacial possuem valor acima da média dos rendimentos do trabalho obtidos por todos os estabelecimentos do país 
...
municipios <- municipios %>%
  ...
  mutate(
    ...
    RENDA_TRABALHO_NAF = (R_VALOR_T_1 + R_VALOR_T_2 + R_VALOR_T_302) - (R_VALOR_AF_1 + R_VALOR_AF_2 + R_VALOR_AF_302),  #rendimentos totais do trabalho
    #RENDAporESTAB_TRABALHO_NAF = ((RENDA_TRABALHO_T - RENDA_TRABALHO_AF) / (NEST_T - NEST_AF)), #rendimentos totais do trabalho, pelo número total de estabelecimentos
    .keep = "used"
  )
...
media_brasilMUN <- municipios %>%
  ...
  mutate(
    ...
    RENDA_TRABALHO_AF = (R_VALOR_AF_1 + R_VALOR_AF_2 + R_VALOR_AF_302),  #rendimentos totais do trabalho
    # RENDAporESTAB_TRABALHO_AF = (RENDA_TRABALHO_AF / NEST_AF), #rendimentos totais do trabalho, pelo número total de estabelecimentos
    RENDA_TRABALHO_NAF = (R_VALOR_T_1 + R_VALOR_T_2 + R_VALOR_T_302) - (R_VALOR_AF_1 + R_VALOR_AF_2 + R_VALOR_AF_302)
  ) %>%
  summarise(
    ...
    REN_BRA_mean_NAF = mean(RENDA_TRABALHO_NAF, na.rm = TRUE),
    ...
    REN_BRA_median_NAF = median(RENDA_TRABALHO_NAF, na.rm = TRUE),
  )
...
microrregioes <- microrregioes %>%
  ...
  mutate(
    ...
    #RENDAporESTAB_TRABALHO_AF = (RENDA_TRABALHO_AF / NEST_AF), #rendimentos totais do trabalho, pelo número total de estabelecimentos
    RENDA_TRABALHO_NAF = (R_VALOR_T_1 + R_VALOR_T_2 + R_VALOR_T_302) - (R_VALOR_AF_1 + R_VALOR_AF_2 + R_VALOR_AF_302),  #rendimentos totais do trabalho
    #RENDAporESTAB_TRABALHO_NAF = ((RENDA_TRABALHO_T - RENDA_TRABALHO_AF) / (NEST_T - NEST_AF)),
    .keep = "used"
  )
...
municipios_class <- municipios_all %>%
  mutate(
    classREN_BRA_AF_T = case_when(
      RENDA_TRABALHO_AF < REN_BRA_median_T ~ "Abaixo da mediana do país",
      RENDA_TRABALHO_AF >= REN_BRA_median_T & RENDA_TRABALHO_AF <= REN_BRA_mean_T ~ "Entre a média e a mediana do país",
      RENDA_TRABALHO_AF > REN_BRA_mean_T ~ "Acima da média do país"),
    
    classREN_BRA_NAF_T = case_when(
      RENDA_TRABALHO_NAF < REN_BRA_median_T ~ "Abaixo da mediana do país",
      RENDA_TRABALHO_NAF >= REN_BRA_median_T & RENDA_TRABALHO_NAF <= REN_BRA_mean_T ~ "Entre a média e a mediana do país",
      RENDA_TRABALHO_NAF > REN_BRA_mean_T ~ "Acima da média do país"))
    ...
    # classREN_BRA_mean_T = case_when(RENDA_TRABALHO_T >= REN_BRA_mean_T ~ "Acima da média do país",
    ...
    #      classREN_BRA_mean_NAFT = case_when(RENDA_TRABALHO_NAF >= REN_BRA_mean_T ~ "Acima da média do país",
    #                                      RENDA_TRABALHO_NAF < REN_BRA_mean_T ~ "Abaixo da média do país"),
    #      
    ...
    #                                      RENDA_TRABALHO_AF < REN_BRA_median_T ~ "Abaixo da mediana do país"),
    #      classREN_BRA_median_NAFT = case_when(RENDA_TRABALHO_NAF >= REN_BRA_median_T ~ "Acima da mediana do país",
    #                                      RENDA_TRABALHO_NAF < REN_BRA_median_T ~ "Abaixo da mediana do país"),
    #      
    #      
...
municipios_class_sumario <- municipios_class %>%
  select(COD_REGIAO, classREN_BRA_AF_T) %>%
  group_by(COD_REGIAO, classREN_BRA_AF_T) %>%
  summarise(n = n()) %>%
  mutate(pct = n/sum(n)) %>%
  mutate(pct = scales::percent(pct, big.mark = ".",  decimal.mark = "," , na.rm = TRUE, accuracy = 0.1)) %>%
  
  pivot_wider(names_from = classREN_BRA_AF_T, 
              values_from = c(n, pct)) %>%
  select(COD_REGIAO,
...
#  kable_minimal() %>%
  ...
  add_header_above(c(" " = 1, "Rendimentos da AF abaixo da mediana do país" = 2, "Rendimentos da AF entre a média e a mediana do país" = 2, "Rendimentos da AF acima da média do país" = 2)) %>%
...
SHP_Municipios_class$classREN_BRA_AF_T <- factor(SHP_Municipios_class$classREN_BRA_AF_T, levels = c("Abaixo da mediana do país", 
  ...
  filter(!is.na(classREN_BRA_AF_T)) %>%
  tm_shape() +
  tm_fill(col = "classREN_BRA_AF_T", 
          palette = cores,
          title = "Rendas do trabalho na AF",
          id = "LOCALIZACAO",
          style = "cat"
          ) +
  tm_borders(lwd = 0.0) +
...
municipios_class_sumario_NAF <- municipios_class %>%
  select(COD_REGIAO, classREN_BRA_NAF_T) %>%
  group_by(COD_REGIAO, classREN_BRA_NAF_T) %>%
  summarise(n = n()) %>%
  mutate(pct = n/sum(n)) %>%
  mutate(pct = scales::percent(pct, big.mark = ".",  decimal.mark = "," , na.rm = TRUE, accuracy = 0.1)) %>%
  
  pivot_wider(names_from = classREN_BRA_NAF_T, 
              values_from = c(n, pct)) %>%
  select(COD_REGIAO,
         `n_Abaixo da mediana do país`,
         `pct_Abaixo da mediana do país`,
...
#  kable_minimal() %>%
  #kable_material(c("striped", "hover")) 
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive")) %>%
  add_header_above(c(" " = 1, "Rendimentos não-AF abaixo da mediana do país" = 2, "Rendimentos não-AF entre a média e a mediana do país" = 2, "Rendimentos não-AF acima da média do país" = 2)) %>%
...
SHP_Municipios_class$classREN_BRA_NAF_T <- factor(SHP_Municipios_class$classREN_BRA_NAF_T, levels = c("Abaixo da mediana do país", 
                                                                                                    "Entre a média e a mediana do país",
                                                                                                    "Acima da média do país")
                                                 )
  ...
  SHP_Municipios_class %>%
  filter(!is.na(classREN_BRA_NAF_T)) %>%
  tm_shape() +
  tm_fill(col = "classREN_BRA_NAF_T", 
          palette = cores,
          title = "Rendas do trabalho não familiar",
          id = "LOCALIZACAO",
          style = "cat"
          ) +
  tm_borders(lwd = 0.0) +
...
SHP_Microrregioes_class$classREN_BRA_AF_T <- factor(SHP_Microrregioes_class$classREN_BRA_AF_T, levels = c("Abaixo da mediana do país", 
                                                                                                    "Entre a média e a mediana do país",
                                                                                                    "Acima da média do país")
                                                 )
...
SHP_Microrregioes_class$classREN_BRA_NAF_T <- factor(SHP_Microrregioes_class$classREN_BRA_NAF_T, levels = c("Abaixo da mediana do país", 
                                                                                                    "Entre a média e a mediana do país",
                                                                                                    "Acima da média do país")
                                                 )
