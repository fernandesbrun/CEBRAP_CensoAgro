...
# REC_PROD - pct tipo 3 ####
REC3_PROD <- 
  microrregioes_pct_pivot3 %>% filter(!is.na(prop3_REC_PROD)) %>% ggplot(aes(x = TIPOLOGIA, y = prop3_REC_PROD,
                 color = TIPOLOGIA, label = LOCALIZACAO)) +
  labs(x = "", y = "Participação", title = "Participação das receitas de produção (animal e vegetal) na composição das rendas", subtitle = "Participação por tipologia nas receitas de produção nas microrregiões") +
  scale_color_brewer(palette = "Dark2", direction=-1, guide = "none") +
  theme_light() +
  theme(legend.position = 'none',
    plot.title = element_text(hjust = 0.5),
...
ggREC2_PROD <- ggplotly(REC3_PROD, tooltip = c("label", "y")) %>%
  layout(margin = margens,
...
* Todas as regiões e tipologias de estabelecimentos possuem média de acima de 50%, ou seja, a maior parte da renda é composta pela produção 
  + É possível verificar que, apesar das médias serem altas, algumas microrregiões possuem valores bastante inferiores a média
* Em todas as regiões é a agricultura familiar que possui menor média, indicando que nessa tipologia outras formas de obtenção de renda também são relevantes na composição do valor total
<br/>
<br/>
