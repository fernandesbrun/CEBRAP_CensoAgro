...
microrregioes_pct <- microrregioes %>%
  mutate(MP_prop_REC_PROD = R_VALOR_MP_1 / R_VALOR_T_1 ,
         ...
         T_VPdivAREA = VP_T / AREA_T, 
         AF_VPdivAREA = VP_AF / AREA_AF,
         MP_VPdivAREA = VP_MP / AREA_MP,
         GP_VPdivAREA = (VP_T - (VP_AF + VP_MP)) / (AREA_T - (AREA_AF + AREA_MP))
...
VPArea <- 
  microrregioes_pct_pivot3 %>% filter(!is.na(VPdivAREA)) %>% filter(VPdivAREA >= 0) %>% filter(VPdivAREA < 10) %>%
...
gg_VP_AREA <- ggplotly(VPArea, tooltip = c("label", "y")) %>%
  layout(margin = margens,
    title = list(text = paste0('Proporção de valor da produção dos estabelecimentos',
                                    '<br>',
                                    '<sup>',
...
## AF
tm_AF_VP_AREA <-
SHP_Micro_join[!is.na(SHP_Micro_join$prop_VP_AREA),] %>%
  filter(prop_VP_AREA >= 0) %>%
  filter(TIPOLOGIA == "AF") %>%
tm_shape() +
  tm_fill(col = "prop_VP_AREA", 
          title = "Valor da produção<br/>nos estabelecimentos<br/>- AF", 
          id = "LOCALIZACAO", palette="BuGn", 
          legend.format=list(fun=function(x) paste0(formatC(x*100, digits=0, format="f"), "%"))) +
  tm_borders(lwd = 0.2) +
...
SHP_Micro_join[!is.na(SHP_Micro_join$prop_VP_AREA),] %>%
  ...
  filter(TIPOLOGIA == "MP") %>%  
tm_shape() +
  tm_fill(col = "prop_VP_AREA", title = "Valor da produção<br/>nos estabelecimentos -<br/>Pronamp", id = "LOCALIZACAO", palette="Oranges",
          legend.format=list(fun=function(x) paste0(formatC(x*100, digits=0, format="f"), "%"))) +
  tm_borders(lwd = 0.2) +
  #tm_facets(by = "TIPOLOGIA", nrow = 1, sync = TRUE, free.coords = TRUE) +
  tm_view(view.legend.position = c("left", "bottom")) +
...
SHP_Micro_join[!is.na(SHP_Micro_join$prop_VP_AREA),] %>%
  ...
  filter(TIPOLOGIA == "GP") %>%
tm_shape() +
  tm_fill(col = "prop_VP_AREA", title = "Valor da produção<br/>nos estabelecimentos -<br/>Outros (patronal)", id = "LOCALIZACAO", palette="Purples",
          legend.format=list(fun=function(x) paste0(formatC(x*100, digits=0, format="f"), "%"))) +
  tm_borders(lwd = 0.2) +
  #tm_facets(by = "TIPOLOGIA", nrow = 1, sync = TRUE, free.coords = TRUE) +
  tm_view(view.legend.position = c("left", "bottom")) +
...
tmap_arrange(tm_AF_VP_AREA, tm_MP_VP_AREA, tm_GP_VP_AREA, 
             sync = TRUE, 
             nrow = 1)
```
