...
pacman::p_load(ggthemes, ggpubr, tidyverse, readr, readxl, esquisse, rstatix, sp, sf, tmap, leaflet, arsenal, knitr, plotly, kableExtra, prettydoc, rmdformats, knitr, scales, broom, cli, crayon , dbplyr, dplyr, forcats, ggplot2, haven, hms, httr, jsonlite, lubridate, magrittr, modelr, pillar, purrr, readr, readxl, reprex, rlang, rstudioapi, rvest, stringr, tibble, tidyr, xml2) 
...
* Estabelecimentos de beneficiados pelo Pronamp - Programa Nacional de Apoio ao Médio Produtor Rural (MP);
  + Proprietários rurais, posseiros, arrendatários ou parceiros que tenham, no mínimo, 80% de sua renda bruta anual originária da atividade agropecuária ou extrativa vegetal e possuam renda bruta anual de até R$ 2 milhões;
* Estabelecimentos não familiares - Agricultura Patronal (AP);
  + Nessa análise, a diferença entre o total dos estabelecimentos e total dos estabelecimentos da agricultura familiar, foi considerada como de tipologia patronal.
  
$$\tiny
Estabelecimentos Patronais = {Estabelecimentos Totais}-{EstabelecimentosAgriculturaFamiliar}
$$
...
Como método de representação visual das características estatísticas e espaciais das variáveis, para cada uma foi elaborado um gráfico e um mapa. 

* Sobre o gráfico: Dividido conforme a grande região de cada microrregião da base de dados original, o gráfico apresenta o desenho da distribuição dos valores para cada uma das regiões e tipos de estabelecimentos, sendo possível observar, portanto, qual a relevância da participação da variável de referência para cada um dos tipos, por regiões brasileiras.
...
* Proporção das rendas totais do produtor beneficiado pelo Pronamp:

$$\tiny
prop.Receita.MP = \frac {Receita.MP} {Receita.T} 
$$

* Proporção das rendas totais da agricultura não familiar (patronal):

$$\tiny
prop.Receita.AP = \frac {Receita.AP} {Receita.T} 
$$
Onde:<br/>
_Receita.AF_ =  Rendas totais (mil reais) nos estabelecimentos da agricultura familiar da microrregião;<br/>
_Receita.MP_ = Rendas totais (mil reais) nos estabelecimentos do produtor beneficiado pelo Pronamp da microrregião;<br/>
_Receita.AP _ =  Rendas totais (mil reais) nos estabelecimentos não familiares, considerados de agricultura patronal;<br/>
...
REC_TOT <- 
  ...
  labs(x = "", y = "", title = "Receitas obtidas") +
  ...
  scale_x_discrete(labels=c("Agricultura não fam. (AP)", "Pronamp", "Agricultura Familiar")) +
  coord_flip()
```
...
* Apenas nos estabelecimentos da Agricultura Familiar da região Nordeste a média das receitas obtidas é maior do que a dos estabelecimentos patronais;
  + Em todas as outras regiões a receita da agricultura patronal é maior do que da Agricultura Familiar ou dos beneficiários do Pronamp;
* Apesar da receita da agricultura patronal ser maior em todas as regiões, exceto no Nordeste, é no Centro-Oeste que essa diferença entre o patronal e os da AF ou do Pronamp é mais exacerbada, com esse tipo de estabelecimento compondo quase 90% da renda da região;
  + É também no Centro-Oeste que a agricultura familiar possui menor média de participação na composição das rendas, com pouco mais de 10%.
<br/>
<br/>
...
SHP_Micro_join[!is.na(SHP_Micro_join$prop_REC_TOT),] %>%
  ...
  filter(TIPOLOGIA == "GP") %>%
tm_shape(bbox = bbox_new) +
  tm_fill(col = "prop_REC_TOT", title = "Receitas obtidas -<br/>Agricultura não fam. (AP)", id = "LOCALIZACAO", palette="Purples",
          legend.format=list(fun=function(x) paste0(formatC(x*100, digits=0, format="f"), "%"))) +
  tm_borders(lwd = 0.2) +
  #tm_facets(by = "TIPOLOGIA", nrow = 1, sync = TRUE, free.coords = TRUE) +
  tm_view(view.legend.position = c("right", "bottom")) +
...
<br/>
<br/>
Abaixo, é possível verificar a distribuição dos quatro grupos de origem da renda a partir de uma média nas microrregiões:

```{r código para a tabela pct, message=FALSE, warning=FALSE, include=FALSE}
library(kableExtra)
library(scales)
...
tabelaRECEITAS[tabelaRECEITAS == "0%"] <- "-"
...
kbl(tabelaRECEITAS, digits = 2, caption = "Origem da renda dos estabelecimentos por tipologia", align = "clccccc") %>%
  ...
  footnote(symbol = "Os estabelecimentos patronais consistem nos estabelecimentos não familiares.",
          footnote_as_chunk = T)
```

Para ver de forma detalhada cada um dos grupos de obtenção de renda, acesse o conteúdo pela abas abaixo.
<br/>

 
##### *Grupo 1* - Receitas da produção do estabelecimento {.tabset .tabset-fade}
<br/>
Receitas da produção do estabelecimento 

###### Proporção da receita de produção pelo total de receitas de cada tipo de estabelecimento

Para o cálculo da proporção das rendas da produção em cada tipo (composição das rendas), foi utilizado:

* Proporção da renda de produção na agricultura familiar:

$$\tiny
prop1.ReceitaProducao.AF = \frac {ReceitaProducao.AF} {Receita.AF} 
$$

* Proporção da renda de produção no produtor beneficiário do Pronamp:

$$\tiny
prop1.ReceitaProducao.MP = \frac {ReceitaProducao.MP} {Receita.MP} 
$$

* Proporção da renda de produção da agricultura não familiar (patronal):

$$\tiny
prop1.ReceitaProducao.AP = \frac {ReceitaProducao.AP} {Receita.AP} 
$$
Onde:<br/>
_ReceitaProducao.AF_ =  Rendas da produção (mil reais) nos estabelecimentos da agricultura familiar da microrregião;<br/>
_ReceitaProducao.MP_ = Rendas da produção (mil reais) nos estabelecimentos do produtor beneficiado pelo Pronamp da microrregião;<br/>
_ReceitaProducao.AP _ =  Rendas da produção (mil reais) nos estabelecimentos não familiares da microrregião (patronais);<br/>
_Receita.AF_ =  Rendas totais (mil reais) nos estabelecimentos da agricultura familiar da microrregião;<br/>
_Receita.MP_ = Rendas totais (mil reais) nos estabelecimentos do produtor beneficiado pelo Pronamp da microrregião;<br/>
_Receita.AP _ =  Rendas totais (mil reais) nos estabelecimentos não familiares (patronais).<br/>

```{r graf RENDAS - Rendas da produção3, message=FALSE, warning=FALSE, include=FALSE}

# REC_PROD - pct tipo 3 ####
REC3_PROD <- 
  microrregioes_pct_pivot3 %>% filter(!is.na(prop3_REC_PROD)) %>% filter(prop3_REC_PROD < 1) %>% 
  ggplot(aes(x = TIPOLOGIA, y = prop3_REC_PROD,
                 color = TIPOLOGIA, label = LOCALIZACAO)) +
  labs(x = "", y = "", title = "Participação das receitas de produção (animal e vegetal) na composição das rendas", subtitle = "Participação por tipologia nas receitas de produção nas microrregiões") +
  scale_color_brewer(palette = "Dark2", direction=-1, guide = "none") +
  theme_light() +
  theme(legend.position = 'none',
    plot.title = element_text(hjust = 0.5),
    plot.subtitle = element_text(hjust = 0.5),
    panel.spacing = unit(1, "lines"),
    axis.title.x = element_text(margin = margin(t = 31, "cm"))) +
  facet_grid(vars(), vars(COD_REGIAO)) +
  geom_violin(fill = "gray80", size = 0.5, alpha = .5) +
  geom_jitter(alpha = .25, width = .3) +
  stat_summary(fun= mean, fun.min=mean, fun.max=mean, geom="crossbar", width=0.8, size = 0.01, color="gray39") +
   scale_y_continuous(breaks=seq(0, 1, by = 0.3), 
                      labels=function(x) paste0(x*100,"%")) +
  scale_x_discrete(labels=c("Agricultura não familiar", "Pronamp", "Agricultura Familiar")) +
  coord_flip()

```

```{r grafPLOT RENDAS - Rendas da produção3 PLOT, echo=FALSE, message=FALSE, warning=FALSE, out.width='100%'}
ggREC3_PROD <- ggplotly(REC3_PROD, tooltip = c("label", "y")) %>%
  layout(margin = margens,
    title = list(text = paste0('Receitas de produção nos estabelecimentos',
                                    '<br>',
                                    '<sup>',
                                    'Participação das receitas de produção (animal e vegetal) na composição das rendas',
                                    '</sup>')))
...
* Todas as regiões e tipologias de estabelecimentos, exceto a agricultura familiar no nordeste, possuem média acima de 50%, ou seja, a maior parte da renda é composta pela produção; <br/>
  + A produção é responsável, em média, por 42% da composição da renda da AF no nordeste; <br/>
  + É possível verificar que, apesar das médias serem altas, algumas microrregiões possuem valores bastante inferiores a média;
* Em todas as regiões é a agricultura familiar que possui menor média, indicando que nessa tipologia outras formas de obtenção de renda também são relevantes na composição do valor total. <br/>
<br/>
<br/>



```{r mapa RENDAS - Rendas da produção3, message=FALSE, warning=FALSE, include=FALSE}
## AF
tm_AF_REC3_PROD <-
SHP_Micro_join[!is.na(SHP_Micro_join$prop3_REC_PROD),] %>%
  filter(prop3_REC_PROD > 0) %>%
  filter(TIPOLOGIA == "AF") %>%
tm_shape(bbox = bbox_new) +
  tm_fill(col = "prop3_REC_PROD", 
          title = "Receita da produção - AF", 
          id = "LOCALIZACAO", palette="BuGn", 
          legend.format=list(fun=function(x) paste0(formatC(x*100, digits=0, format="f"), "%")),
          style = "quantile") +
  tm_borders(lwd = 0.2) +
...
SHP_Micro_join[!is.na(SHP_Micro_join$prop3_REC_PROD),] %>%
  ...
  filter(TIPOLOGIA == "MP") %>%  
tm_shape(bbox = bbox_new) +
  tm_fill(col = "prop3_REC_PROD", title = "Receita da produção -<br/>Pronamp", id = "LOCALIZACAO", palette="Oranges",
          legend.format=list(fun=function(x) paste0(formatC(x*100, digits=0, format="f"), "%")),
          style = "quantile") +
  tm_borders(lwd = 0.2) +
  #tm_facets(by = "TIPOLOGIA", nrow = 1, sync = TRUE, free.coords = TRUE) +
...
SHP_Micro_join[!is.na(SHP_Micro_join$prop3_REC_PROD),] %>%
  filter(prop3_REC_PROD > 0) %>%
  filter(prop3_REC_PROD < 1) %>%
  filter(TIPOLOGIA == "GP") %>%
tm_shape(bbox = bbox_new) +
  tm_fill(col = "prop3_REC_PROD", title = "Receita da produção -<br/>Agricultura não fam. (AP)", id = "LOCALIZACAO", palette="Purples",
          legend.format=list(fun=function(x) paste0(formatC(x*100, digits=0, format="f"), "%"))) +
  tm_borders(lwd = 0.2) +
  #tm_facets(by = "TIPOLOGIA", nrow = 1, sync = TRUE, free.coords = TRUE) +
  tm_view(view.legend.position = c("right", "bottom")) +
...
```{r mapaPLOT RENDAS - Rendas da produção3 PLOT, echo=FALSE, message=FALSE, warning=FALSE}
tmap_arrange(tm_AF_REC3_PROD, tm_MP_REC3_PROD, tm_GP_REC3_PROD, 
             sync = TRUE, 
             nrow = 1)
```
<br/>
...
###### Proporção da receita de produção por tipo de estabelecimento, pelo total da produção

Para o cálculo da proporção das rendas da produção em cada tipo, ou seja, quanto cada tipo de estabelecimento contribui na composição do total das receitas de produção, foi utilizado:

*Proporção das rendas de produção advindas da agricultura familiar:

$$\tiny
prop2.ReceitaProducao.AF = \frac {ReceitaProducao.AF} {ReceitaProducao.T} 
$$

* Proporção das rendas de produção advindas do produtor beneficiário do Pronamp:

$$\tiny
prop2.ReceitaProducao.MP = \frac {ReceitaProducao.MP} {ReceitaProducao.T} 
$$

* Proporção das rendas de produção advindas da agricultura não familiar (patronal):

$$\tiny
prop2.ReceitaProducao.AP = \frac {ReceitaProducao.AP} {ReceitaProducao.T} 
$$
Onde:<br/>
_ReceitaProducao.AF_ =  Rendas da produção (mil reais) nos estabelecimentos da agricultura familiar da microrregião;<br/>
_ReceitaProducao.MP_ = Rendas da produção (mil reais) nos estabelecimentos do produtor beneficiado pelo Pronamp da microrregião;<br/>
_ReceitaProducao.AP _ =  Rendas da produção (mil reais) nos estabelecimentos não familiares (patronais);<br/>
...
REC_PROD <- 
  microrregioes_pct_pivot3 %>% filter(!is.na(prop_REC_PROD)) %>% ggplot(aes(x = TIPOLOGIA, y = prop_REC_PROD,
                 color = TIPOLOGIA, label = LOCALIZACAO)) +
  labs(x = "", y = "", title = "Receitas de produção (animal e vegetal)", subtitle = "Participação por tipologia nas receitas de produção nas microrregiões") +
  scale_color_brewer(palette = "Dark2", direction=-1, guide = "none") +
  theme_light() +
  theme(legend.position = 'none',
    plot.title = element_text(hjust = 0.5),
...
* Três regiões (Nordeste, Norte, Sul) apresentam a média da participação das receitas obtidas nos estabelecimentos de AF mais próximo da média das outras duas tipologias, apesar de ainda inferiores;
...
* Enquando os dados do Centro-Oeste apontam para uma dominância da agricultura patronal (>90% das receitas obtidas em estabelecimentos patronais, ~30% de beneficiários do Pronamp e ~10% de agricultura familiar);
	+ Em contraponto, a região Nordeste demonstra uma variância nas categorias que vai na contra-mão do Centro-Oeste, com ~45% das receitas tendo sido obtidas pela agricultura familiar, contra ~55% da patronal;
* Agricultura familiar na região norte também recebe destaque, apresentando uma diferença ainda menor entre a agricultura familiar e a não familiar (patronal), com médias de ~47% e 52%, respectivamente;
...
SHP_Micro_join[!is.na(SHP_Micro_join$prop_REC_PROD),] %>%
  ...
  filter(TIPOLOGIA == "GP") %>%
tm_shape(bbox = bbox_new) +
  tm_fill(col = "prop_REC_PROD", title = "Receitas da produção<br/>do estabelecimento -<br/>Agricultura não fam. (AP)", id = "LOCALIZACAO", palette="Purples",
          legend.format=list(fun=function(x) paste0(formatC(x*100, digits=0, format="f"), "%"))) +
  tm_borders(lwd = 0.2) +
  #tm_facets(by = "TIPOLOGIA", nrow = 1, sync = TRUE, free.coords = TRUE) +
  tm_view(view.legend.position = c("right", "bottom")) +
...
##### *Grupo 2* - Outras receitas do estabelecimento {.tabset .tabset-fade}
<br/>
Outras receitas do estabelecimento

###### Proporção de outras receitas pelo total de receitas de cada tipo de estabelecimento
 
Para o cálculo da proporção das outras rendas do estabelecimento em cada tipo (composição das rendas), foi utilizado:

* Proporção de outras receitas do estabelecimento na agricultura familiar:

$$\tiny
prop1.ReceitaEstabelecimento.AF = \frac {ReceitaEstabelecimentoOutras.AF} {Receita.AF} 
$$

* Proporção de outras receitas do estabelecimento do produtor beneficiário do Pronamp:

$$\tiny
prop1.ReceitaEstabelecimento.MP = \frac {ReceitaEstabelecimentoOutras.MP} {Receita.MP} 
$$

* Proporção de outras receitas do estabelecimento não familiar (patronal):

$$\tiny
prop1.ReceitaEstabelecimento.AP = \frac {ReceitaEstabelecimentoOutras.AP} {Receita.AP} 
$$
Onde:<br/>
_ReceitaEstabelecimentoOutras.AF_ =  Outras rendas do estabelecimento (mil reais) nos estabelecimentos da agricultura familiar da microrregião;<br/>
_ReceitaEstabelecimentoOutras.MP_ = Outras rendas do estabelecimento (mil reais) nos estabelecimentos do produtor beneficiado pelo Pronamp da microrregião;<br/>
_ReceitaEstabelecimentoOutras.AP _ = Outras rendas do estabelecimento (mil reais) nos estabelecimentos que não da agricultura familiar (patronais);<br/>
_ReceitaEstabelecimento.T_ = Rendas do estabelecimento (mil reais) nos estabelecimentos agropecuários na microrregião; <br/>
_Receita.AF_ =  Rendas totais (mil reais) nos estabelecimentos da agricultura familiar da microrregião;<br/>
_Receita.MP_ = Rendas totais (mil reais) nos estabelecimentos do produtor beneficiado pelo Pronamp da microrregião;<br/>
_Receita.AP _ =  Rendas totais (mil reais) nos estabelecimentos que não são da agricultura familiar (patronais).<br/>


```{r graf RENDAS - Rendas outras3, message=FALSE, warning=FALSE, include=FALSE}
# REC_OUTR - pct tipo 1 ####
REC3_OUTR <- 
  microrregioes_pct_pivot3 %>% filter(!is.na(prop3_REC_OUTR)) %>% filter(prop3_REC_OUTR < 1) %>% 
  ggplot(aes(x = TIPOLOGIA, y = prop3_REC_OUTR, color = TIPOLOGIA, label = LOCALIZACAO)) +
  labs(x = "", y = "", title = "Outras* receitas do estabelecimento", subtitle = "Participação por tipologia outras* receitas do estabelecimento", caption = "Outras receitas do estabelecimento: desinvestimentos, serviço de turismo rural, exploração mineral, atividade de artesanato, tecelagem, etc") +
  scale_color_brewer(palette = "Dark2", direction=-1, guide = "none") +
  theme_light() +
  theme(legend.position = 'none',
        plot.title = element_text(hjust = 0.5),
...
```{r grafPLOT RENDAS - Rendas outras3 PLOT, echo=FALSE, message=FALSE, warning=FALSE, out.width='100%'}
ggREC3_OUTR <- ggplotly(REC3_OUTR, tooltip = c("label", "y")) %>%
  layout(margin = margens,
    title = list(text = paste0('Outras* receitas do estabelecimento',
                                    '<br>',
                                    '<sup>',
                                    'Participação das outras receitas do estabelecimento na composição das rendas',
                                    '</sup>')))
...
* Embora o valor advindo de outras receitas no estabelecimento seja baixo na composição da renda da maior parte dos estabelecimentos das microrregiões, algumas poucas localidades demonstram valores mais elevados que a média, podendo indicar de forma bastante específica que aquela microrregião explora melhor outras atividades possíveis para a obtenção de renda. 
	- As médias são muito similares para as cinco regiões, em todos os tipos de estabelecimentos; 
* Pelas porcentagens serem muito pequenas (menos de 5%) no total das receitas de cada um, a categoria não se mostra tão significativa, exceto para aquelas microrregiões específicas que apresentam valores altos; 
* Apesar dos rendimentos de outras atividades do estabelecimento terem alguns pontos específicos em que essa renda compõe a renda total dos estabelecimentos de forma considerável, não é possível observar uma rede entre microrregiões.
	
	<br/>
<br/>
<br/>

```{r mapa RENDAS - Rendas outras3, message=FALSE, warning=FALSE, include=FALSE}
## AF
tm_AF_REC3_OUTR <-
SHP_Micro_join[!is.na(SHP_Micro_join$prop3_REC_OUTR),] %>%
  filter(prop3_REC_OUTR > 0) %>%
  filter(prop3_REC_OUTR < 1) %>%
  filter(TIPOLOGIA == "AF") %>%
tm_shape(bbox = bbox_new) +
  tm_fill(col = "prop3_REC_OUTR",
          title = "Outras receitas<br/>do estabelecimento - AF",
          id = "LOCALIZACAO", palette="BuGn",
          legend.format=list(fun=function(x) paste0(formatC(x*100, digits=0, format="f"), "%"))) +
  tm_borders(lwd = 0.2) +
  #tm_facets(by = "TIPOLOGIA", nrow = 1, sync = TRUE, free.coords = TRUE) +
...
SHP_Micro_join[!is.na(SHP_Micro_join$prop3_REC_OUTR),] %>%
  ...
  filter(TIPOLOGIA == "MP") %>%
tm_shape(bbox = bbox_new) +
  tm_fill(col = "prop3_REC_OUTR", title = "Outras receitas<br/>do estabelecimento -<br/>Pronamp", id = "LOCALIZACAO", palette="Oranges",
          legend.format=list(fun=function(x) paste0(formatC(x*100, digits=0, format="f"), "%"))) +
  tm_borders(lwd = 0.2) +
  #tm_facets(by = "TIPOLOGIA", nrow = 1, sync = TRUE, free.coords = TRUE) +
  tm_view(view.legend.position = c("right", "bottom")) +
...
SHP_Micro_join[!is.na(SHP_Micro_join$prop3_REC_OUTR),] %>%
  ...
  filter(TIPOLOGIA == "GP") %>%
tm_shape(bbox = bbox_new) +
  tm_fill(col = "prop3_REC_OUTR", title = "Outras receitas<br/>do estabelecimento -<br/>Agricultura Patronal (AP)", id = "LOCALIZACAO", palette="Purples",
          legend.format=list(fun=function(x) paste0(formatC(x*100, digits=0, format="f"), "%"))) +
  tm_borders(lwd = 0.2) +
  #tm_facets(by = "TIPOLOGIA", nrow = 1, sync = TRUE, free.coords = TRUE) +
  tm_view(view.legend.position = c("right", "bottom")) +
...
<!-- Devido a baixa manifestação espacial de microrregiões no mapa, essa visualização não foi gerada. -->

```{r mapaPLOT RENDAS - Rendas outras3 PLOT, echo=FALSE, message=FALSE, warning=FALSE}
tmap_arrange(tm_AF_REC3_OUTR, tm_MP_REC3_OUTR, tm_GP_REC3_OUTR,
             sync = TRUE,
             nrow = 1)
```
...
###### Proporção de outras receitas por tipo de estabelecimento, pelo total da mesma categoria

Para o cálculo da proporção das outras rendas em cada tipo, ou seja, quanto cada tipo de estabelecimento contribui na composição do total das outras rendas do estabelecimento, foi utilizado:

* Proporção das rendas do estabelecimento da agricultura familiar:

$$\tiny
prop2.ReceitaEstabelecimento.AF = \frac {ReceitaEstabelecimento.AF} {ReceitaEstabelecimento.T} 
$$

* Proporção das rendas do estabelecimento do produtor beneficiário do Pronamp:

$$\tiny
prop2.ReceitaEstabelecimento.MP = \frac {ReceitaEstabelecimento.MP} {ReceitaEstabelecimento.T} 
$$

* Proporção das rendas do estabelecimento não familiar (patronal):

$$\tiny
prop2.ReceitaEstabelecimento.AP = \frac {ReceitaEstabelecimento.AP} {ReceitaEstabelecimento.T} 
$$
Onde:<br/>
...
_ReceitaEstabelecimento.MP_ = Rendas do estabelecimento (mil reais) nos estabelecimentos do produtor beneficiado pelo Pronamp da microrregião;<br/>
_ReceitaEstabelecimento.AP _ =  Rendas do estabelecimento (mil reais) nos estabelecimentos que não são de agricultura familiar (patronais);<br/>
_ReceitaEstabelecimento.T_ = Rendas do estabelecimento (mil reais) nos estabelecimentos agropecuários na microrregião. <br/>
...
ggREC_OUTR <- ggplotly(REC_OUTR, tooltip = c("label", "y")) %>%
  layout(margin = margens,
    title = list(text = paste0('Outras* receitas do estabelecimento',
                                    '<br>',
                                    '<sup>',
                                    'Participação por tipologia nas outras receitas do estabelecimento nas microrregiões',
                                    '</sup>')))
...
* No Sul, Norte e Nordeste a participação da agricultura familiar na composição das outras rendas dos estabelecimentos é superior às outras tipologias;
* No Nordeste as microrregiões de destaque na categoria de agricultura familiar são as litorâneas, enquanto que as patronais são as interioranas, especialmente no Centro-Oeste.
<br/>
<br/>
...
SHP_Micro_join[!is.na(SHP_Micro_join$prop_REC_OUTR),] %>%
  ...
  filter(TIPOLOGIA == "MP") %>%  
tm_shape(bbox = bbox_new) +
  tm_fill(col = "prop_REC_OUTR", title = "Outras receitas<br/>do estabelecimento -<br/>Pronamp", id = "LOCALIZACAO", palette="Oranges",
          legend.format=list(fun=function(x) paste0(formatC(x*100, digits=0, format="f"), "%"))) +
  tm_borders(lwd = 0.2) +
  #tm_facets(by = "TIPOLOGIA", nrow = 1, sync = TRUE, free.coords = TRUE) +
  tm_view(view.legend.position = c("right", "bottom")) +
...
SHP_Micro_join[!is.na(SHP_Micro_join$prop_REC_OUTR),] %>%
  ...
  filter(TIPOLOGIA == "GP") %>%
tm_shape(bbox = bbox_new) +
  tm_fill(col = "prop_REC_OUTR", title = "Outras receitas<br/>do estabelecimento -<br/>Agricultura Patronal", id = "LOCALIZACAO", palette="Purples",
          legend.format=list(fun=function(x) paste0(formatC(x*100, digits=0, format="f"), "%"))) +
  tm_borders(lwd = 0.2) +
  #tm_facets(by = "TIPOLOGIA", nrow = 1, sync = TRUE, free.coords = TRUE) +
  tm_view(view.legend.position = c("right", "bottom")) +
...
##### *Grupo 3* - Receitas de atividades fora do estabelecimento {.tabset .tabset-fade}
<br/>
Receitas de atividades fora do estabelecimento

###### Proporção de rendas obtidas fora do estabelecimento, pelo total de receitas de cada tipo de estabelecimento
...
$$\tiny
prop.ReceitaFora.AF = \frac {ReceitaFora.AF} {Receita.AF} 
$$

* Proporção das rendas de atividades fora do estabelecimento do produtor beneficiário do Pronamp:

$$\tiny
prop.ReceitaFora.MP = \frac {ReceitaFora.MP} {Receita.MP} 
$$

* Proporção das rendas de atividades fora do estabelecimento não familiar (patronal):

$$\tiny
prop.ReceitaFora.AP = \frac {ReceitaFora.AP} {Receita.AP} 
$$
Onde:<br/>
...
_ReceitaFora.MP_ = rendas de Rendas de atividades fora do estabelecimento (mil reais) nos estabelecimentos do produtor beneficiado pelo Pronamp da microrregião;<br/>
_ReceitaFora.AP _ =  rendas de Rendas de atividades fora do estabelecimento (mil reais) nos estabelecimentos que não são nem agricultura familiar, nem médio produtor da microrregião (patronais);<br/>
...
_Receita.AF_ =  Rendas totais (mil reais) nos estabelecimentos da agricultura familiar da microrregião;<br/>
_Receita.MP_ = Rendas totais (mil reais) nos estabelecimentos do produtor beneficiado pelo Pronamp da microrregião;<br/>
_Receita.AP _ =  Rendas totais (mil reais) nos estabelecimentos que não são de agricultura familiar (patronais).<br/>



```{r graf RENDAS - fora3, message=FALSE, warning=FALSE, include=FALSE}
# prop3_REC_FORA - pct tipo 1 ####
REC3_FORA <- 
  microrregioes_pct_pivot3 %>% filter(!is.na(prop3_REC_FORA)) %>% filter(prop3_REC_FORA < 1) %>% 
  ggplot(aes(x = TIPOLOGIA, y = prop3_REC_FORA, color = TIPOLOGIA, label = LOCALIZACAO)) +
  labs(x = "", y = "", title = "Receitas obtidas fora do estabelecimento", subtitle = "Participação por tipologia de outras receitas do estabelecimento", caption = "* Outras receitas do estabelecimento: desinvestimentos, serviço de turismo rural, exploração mineral, atividade de artesanato, tecelagem, etc") +
  scale_color_brewer(palette = "Dark2", direction=-1, guide = "none") +
  theme_light() +
  theme(legend.position = 'none',
        plot.title = element_text(hjust = 0.5),
...
```{r grafPLOT RENDAS - Rendas FORA3 PLOT, echo=FALSE, message=FALSE, warning=FALSE, out.width='100%'}
ggREC3_FORA <- ggplotly(REC3_FORA, tooltip = c("label", "y")) %>%
  layout(margin = margens,
    title = list(text = paste0('Receitas obtidas fora do estabelecimento',
                                    '<br>',
                                    '<sup>',
                                    'Participação das receitas obtidas fora do estabelecimento na composição das rendas',
                                    '</sup>')))
...
* Em todas as regiões, é o estabelecimento do médio produtor que possui média mais alta de obtenção de receitas fora do estabelecimento na composição da renda;<br/>
* Em todas as regiões, a média da agricultura familiar é muito baixa; <br/>
* O tipo de estabelecimento com resultados mais dispersos é o do produtor não familiar (patronal), especialmente no Nordeste.<br/>
<br/>
<br/>

```{r mapa RENDAS - Rendas fora3, message=FALSE, warning=FALSE, include=FALSE}
## AF
tm_AF_REC3_FORA <-
SHP_Micro_join[!is.na(SHP_Micro_join$prop3_REC_FORA),] %>%
  filter(prop3_REC_FORA > 0) %>%
  filter(prop3_REC_FORA < 1) %>%
  filter(TIPOLOGIA == "AF") %>%
tm_shape(bbox = bbox_new) +
  tm_fill(col = "prop3_REC_FORA", 
          title = "Receitas fora<br/>do estabelecimento - AF", 
          id = "LOCALIZACAO", palette="BuGn", 
          legend.format=list(fun=function(x) paste0(formatC(x*100, digits=0, format="f"), "%"))) +
  tm_borders(lwd = 0.2) +
  #tm_facets(by = "TIPOLOGIA", nrow = 1, sync = TRUE, free.coords = TRUE) +
...
SHP_Micro_join[!is.na(SHP_Micro_join$prop3_REC_FORA),] %>%
  ...
  filter(TIPOLOGIA == "MP") %>%  
tm_shape(bbox = bbox_new) +
  tm_fill(col = "prop3_REC_FORA", title = "Receitas fora<br/>do estabelecimento -<br/>Pronamp", id = "LOCALIZACAO", palette="Oranges",
          legend.format=list(fun=function(x) paste0(formatC(x*100, digits=0, format="f"), "%")),
          style = "kmeans") +
  tm_borders(lwd = 0.2) +
  #tm_facets(by = "TIPOLOGIA", nrow = 1, sync = TRUE, free.coords = TRUE) +
...
SHP_Micro_join[!is.na(SHP_Micro_join$prop3_REC_FORA),] %>%
  ...
  filter(TIPOLOGIA == "GP") %>%
tm_shape(bbox = bbox_new) +
  tm_fill(col = "prop3_REC_FORA", title = "Receitas fora<br/>do estabelecimento -<br/>Agricultura Patronal", id = "LOCALIZACAO", palette="Purples",
          legend.format=list(fun=function(x) paste0(formatC(x*100, digits=0, format="f"), "%")),
          style = "kmeans") +
  tm_borders(lwd = 0.2) +
  #tm_facets(by = "TIPOLOGIA", nrow = 1, sync = TRUE, free.coords = TRUE) +
...
```
Mapa
```{r mapaPLOT RENDAS - Rendas FORA3 PLOT, echo=FALSE, message=FALSE, warning=FALSE}

tmap_arrange(tm_AF_REC3_FORA, tm_MP_REC3_FORA, tm_GP_REC3_FORA, 
             sync = TRUE, 
             nrow = 1)
...
###### Proporção de rendas obtidas fora do estabelecimento por tipo, pelo total da mesma categoria

Para o cálculo da proporção das rendas fora do estabelecimento em cada tipo, ou seja, quanto cada tipo de estabelecimento contribui na composição do total das rendas fora do estabelecimento, foi utilizado:


* Proporção das rendas de atividades fora do estabelecimento na agricultura familiar:

$$\tiny
prop2.ReceitaFora.AF = \frac {ReceitaFora.AF} {ReceitaFora.T} 
$$

* Proporção das rendas de atividades fora do estabelecimento do produtor beneficiário do Pronamp:

$$\tiny
prop2.ReceitaFora.MP = \frac {ReceitaFora.MP} {ReceitaFora.T} 
$$

* Proporção das rendas de atividades fora do estabelecimento não familiar (patronal):

$$\tiny
prop2.ReceitaFora.AP = \frac {ReceitaFora.AP} {ReceitaFora.T} 
$$
Onde:<br/>
_ReceitaFora.AF_ =  rendas de Rendas de atividades fora do estabelecimento (mil reais) nos estabelecimentos da agricultura familiar da microrregião;<br/>
_ReceitaFora.MP_ = rendas de Rendas de atividades fora do estabelecimento (mil reais) nos estabelecimentos do produtor beneficiado pelo Pronamp da microrregião;<br/>
_ReceitaFora.AP _ =  rendas de Rendas de atividades fora do estabelecimento (mil reais) nos estabelecimentos que não são de agricultura familiar (patronais);<br/>
_ReceitaFora.T_ = rendas de Rendas de atividades fora do estabelecimento (mil reais) nos estabelecimentos agropecuários na microrregião. <br/>
...
REC_FORA <- 
  ...
  labs(x = "", y = "", title = "Rendas obtidas em atividades fora do estabelecimento", subtitle = "Participação por tipologia nas rendas obtidas em atividades fora do estabelecimento") +
  scale_color_brewer(palette = "Dark2", direction=-1, guide = "none") +
  theme_light() +
  theme(legend.position = 'none',
        plot.title = element_text(hjust = 0.5),
  ...
  scale_y_continuous(limits=c(0,1), breaks=seq(0, 1, by = 0.3), 
                     labels=function(x) paste0(x*100,"%")) +
  scale_x_discrete(labels=c("Agricultura não familiar (patronal)", "Pronamp", "Agricultura Familiar")) +
  coord_flip()
```
...
ggREC_FORA <- ggplotly(REC_FORA, tooltip = c("label", "y")) %>%
  layout(margin = margens,
    title = list(text = paste0('Rendas obtidas fora do estabelecimento',
                                    '<br>',
                                    '<sup>',
                                    'Participação por tipologia nas rendas obtidas em atividades fora do estabelecimento nas microrregiões',
                                    '</sup>')))
...
Questões principais:
* Nessa seção, apesar da AP não ter ocupado um papel de tanto destaque quando se trata das rendas advindas de fora do estabelecimento na composição total da renda do estabelecimento, essa tipologia é a que apresenta porcentagens mais altas no total de rendas obtidas nessas atividades;
* Os estabelecimentos do médio produtor possuem valores valores acima de 50% em todas as regiões, com grande destaque para o sertão nordestino, com média de 80% de participação da composição da renda dos estabelecimentos agropecuários nas microrregiões adivinda de atividades fora do estabelecimento;
...
SHP_Micro_join[!is.na(SHP_Micro_join$prop_REC_FORA),] %>%
  ...
  filter(TIPOLOGIA == "GP") %>%
tm_shape(bbox = bbox_new) +
  tm_fill(col = "prop_REC_FORA", title = "Receitas obtidas fora<br/>do estabelecimento -<br/>Agricultura Patronal", id = "LOCALIZACAO", palette="Purples",
          legend.format=list(fun=function(x) paste0(formatC(x*100, digits=0, format="f"), "%"))) +
  tm_borders(lwd = 0.2) +
  #tm_facets(by = "TIPOLOGIA", nrow = 1, sync = TRUE, free.coords = TRUE) +
  tm_view(view.legend.position = c("right", "bottom")) +
...
##### *Grupo 4* - Outras receitas do produtor {.tabset .tabset-fade}
<br/>
Outras receitas do produtor

###### Proporção de outras receitas do produtor pelo total de receitas de cada tipo de estabelecimento

As outras receitas do produtor são compostas por programas sociais, aposentadorias e prêmios.
Para o cálculo da proporção das outras rendas do produtor em cada tipo de estabelecimento (composição das rendas), foi utilizado:

* Proporção de outras receitas do produtor na agricultura familiar:

$$\tiny
prop1.ReceitaProgPoli.AF = \frac {ReceitaProgPoli.AF} {Receita.AF} 
$$

* Proporção de outras receitas do produtor para o produtor beneficiário do Pronamp:

$$\tiny
prop1.ReceitaProgPoli.MP = \frac {ReceitaProgPoli.AF} {Receita.MP} 
$$

* Proporção das rendas de atividades fora do estabelecimento não familiar (patronal):

$$\tiny
prop1.ReceitaProgPoli.AP = \frac {ReceitaProgPoli.AF} {Receita.AP} 
$$
Onde:<br/>
...
_ReceitaProgPoli.MP_ = Rendas de programas e políticas (mil reais) do produtor nos estabelecimentos do produtor beneficiado pelo Pronamp da microrregião;<br/>
_ReceitaProgPoli.AP _ =  Rendas de programas e políticas (mil reais) do produtor nos estabelecimentos que não são de agricultura familiar (patronais);<br/>
...
```{r graf RENDAS - PROG3, message=FALSE, warning=FALSE, include=FALSE}
# prop3_REC_FORA - pct tipo 1 ####
REC3_PROG <- 
  microrregioes_pct_pivot3 %>% filter(!is.na(prop3_REC_PROG)) %>% filter(prop3_REC_PROG < 1) %>% 
  ggplot(aes(x = TIPOLOGIA, y = prop3_REC_PROG, color = TIPOLOGIA, label = LOCALIZACAO)) +
  labs(x = "", y = "", title = "Receitas de programas e políticas", subtitle = "Participação das receitas obtidas em programas sociais, aposentadorias e prêmios na composição das rendas") +
  scale_color_brewer(palette = "Dark2", direction=-1, guide = "none") +
  theme_light() +
  theme(legend.position = 'none',
        plot.title = element_text(hjust = 0.5),
...
```{r grafPLOT RENDAS - Rendas PROG3 PLOT, echo=FALSE, message=FALSE, warning=FALSE, out.width='100%'}
ggREC3_PROG <- ggplotly(REC3_PROG, tooltip = c("label", "y")) %>%
  layout(margin = margens,
    title = list(text = paste0('Receitas obtidas por programas e políticas',
                                    '<br>',
                                    '<sup>',
                                    'Participação das receitas obtidas em programas sociais, aposentadorias e prêmios na composição das rendas',
                                    '</sup>')))
...
* As rendas obtidas em programas e políticas são maiores no tipo de estabelecimento da agricultura familiar em todas as regiões e preticamente em todas as microrregiões; <br/>
* Na região Nordeste a média é maior que o mesmo de cada tipo em outras regiões para todos os tipos de estabelecimento
  + Na região nordeste, a agricultura familiar é a única que possui média que ultrapassa 50%, ou seja, a composição principal da renda nos estabelecimentos de AF das microrregiões do nordestes é dada por programas sociais, aposentadorias ou prêmios. <br/>
<br/>
<br/>
<br/>

```{r mapa RENDAS - Rendas PROG3, message=FALSE, warning=FALSE, include=FALSE}
## AF
tm_AF_REC3_PROG <-
SHP_Micro_join[!is.na(SHP_Micro_join$prop3_REC_PROG),] %>%
  filter(prop3_REC_PROG > 0) %>%
  filter(prop3_REC_PROG < 1) %>%
  filter(TIPOLOGIA == "AF") %>%
tm_shape(bbox = bbox_new) +
  tm_fill(col = "prop3_REC_PROG", 
          title = "Receitas de<br/>programas e políticas - AF", 
          id = "LOCALIZACAO", palette="BuGn", 
          legend.format=list(fun=function(x) paste0(formatC(x*100, digits=0, format="f"), "%")),
          style = "quantile") +
  tm_borders(lwd = 0.2) +
...
SHP_Micro_join[!is.na(SHP_Micro_join$prop3_REC_PROG),] %>%
  ...
  filter(TIPOLOGIA == "MP") %>%  
tm_shape(bbox = bbox_new) +
  tm_fill(col = "prop3_REC_PROG", title = "Receitas de<br/>programas e políticas -<br/>Pronamp", id = "LOCALIZACAO", palette="Oranges",
          legend.format=list(fun=function(x) paste0(formatC(x*100, digits=0, format="f"), "%")),
          style = "quantile") +
  tm_borders(lwd = 0.2) +
  #tm_facets(by = "TIPOLOGIA", nrow = 1, sync = TRUE, free.coords = TRUE) +
...
SHP_Micro_join[!is.na(SHP_Micro_join$prop3_REC_PROG),] %>%
  ...
  filter(TIPOLOGIA == "GP") %>%
tm_shape(bbox = bbox_new) +
  tm_fill(col = "prop3_REC_PROG", title = "Receitas de<br/>programas e políticas -<br/>Agricultura Patronal", id = "LOCALIZACAO", palette="Purples",
          legend.format=list(fun=function(x) paste0(formatC(x*100, digits=0, format="f"), "%")),
          style = "fixed", breaks = c(0, 0, 0.01, 0.05, 0.25)) +
  tm_borders(lwd = 0.2) +
  #tm_facets(by = "TIPOLOGIA", nrow = 1, sync = TRUE, free.coords = TRUE) +
  tm_view(view.legend.position = c("right", "bottom")) +
  tm_shape(SHP_Estados) +
...
```{r mapaPLOT RENDAS - Rendas PROG3 PLOT, echo=FALSE, message=FALSE, warning=FALSE}
tmap_arrange(tm_AF_REC3_PROG, tm_MP_REC3_PROG, #tm_GP_REC3_PROG,
             sync = TRUE,
             nrow = 1)

```
<br/>
<!-- Devido a baixa manifestação espacial de microrregiões para os estabelecimentos patronais, não foi gerada essa visualização em mapa. -->
<br/>
<br/>


###### Proporção de outras rendas do produtor por tipo, pelo total das receitas da categoria

As outras receitas do produtor são compostas por programas sociais, aposentadorias e prêmios.
Para o cálculo da proporção das outras rendas do produtor em cada tipo, ou seja, quanto cada tipo de estabelecimento contribui na composição do total das outras rendas do produtor, foi utilizado:


* Proporção das rendas de programas e políticas do produtor na agricultura familiar:

$$\tiny
prop2.ReceitaProgPoli.AF = \frac {ReceitaProgPoli.AF} {ReceitaProgPoli.T} 
$$

* Proporção das rendas de programas e políticas do produtor beneficiário do Pronamp:

$$\tiny
prop2.ReceitaProgPoli.MP = \frac {ReceitaProgPoli.MP} {ReceitaProgPoli.T} 
$$

* Proporção das rendas de programas e políticas do produtor da agricultura não familiar (patronal):

$$\tiny
prop2.ReceitaProgPoli.AP = \frac {ReceitaProgPoli.AP} {ReceitaProgPoli.T} 
$$
Onde:<br/>
_ReceitaProgPoli.AF_ =  Rendas de programas e políticas (mil reais) do produtor nos estabelecimentos da agricultura familiar da microrregião;<br/>
_ReceitaProgPoli.MP_ = Rendas de programas e políticas (mil reais) do produtor nos estabelecimentos do produtor beneficiado pelo Pronamp da microrregião;<br/>
...
REC_PROG <- 
  ...
  labs(x = "", y = "", title = "Outras receitas do produtor", subtitle = "Participação por tipologia em outras receitas do produtor (programas sociais/aposentadorias/premios)") +
  scale_color_brewer(palette = "Dark2", direction=-1, guide = "none") +
  theme_light() +
  theme(legend.position = 'none',
        plot.title = element_text(hjust = 0.5),
...
* Agricultura familiar é a que utiliza quase 100% da renda desta categoria, isso indica que há uma destinação maior de recursos de programas e políticas para todos os estabelecimentos desse tipo nas regiões;
* Esse tipo de obtenção de renda tem pouca variância entre os estabelecimentos de microrregiões de mesmo tipo, ou seja, em todas as regiões os números para os três tipos de estabelecimentos são muito próximos, indicando um certo uníssono, quando uma microrregião utiliza o tipo de composição de renda, é provável que as do entorno também utilizarão.<br/>
<br/>

<!-- Devido a baixa manifestação espacial de microrregiões no mapa, não foi gerada essa visualização. -->
...
tm_shape(bbox = bbox_new) +
  tm_fill(col = "prop_REC_PROG",
          title = "Outras receitas do produtor<br/>- AF",
          id = "LOCALIZACAO", palette="BuGn",
          legend.format=list(fun=function(x) paste0(formatC(x*100, digits=0, format="f"), "%")),
          n = 3,
          style = "quantile") +
...
SHP_Micro_join[!is.na(SHP_Micro_join$prop_REC_PROG),] %>%
  ...
  filter(TIPOLOGIA == "MP") %>%
tm_shape(bbox = bbox_new) +
  tm_fill(col = "prop_REC_PROG", title = "Outras receitas do produtor-<br/>Pronamp", id = "LOCALIZACAO", palette="Oranges",
          legend.format=list(fun=function(x) paste0(formatC(x*100, digits=0, format="f"), "%")),
          n = 3,
          style = "quantile") +
  tm_borders(lwd = 0.2) +
...
SHP_Micro_join[!is.na(SHP_Micro_join$prop_REC_PROG),] %>%
  ...
  filter(TIPOLOGIA == "GP") %>%
tm_shape(bbox = bbox_new) +
  tm_fill(col = "prop_REC_PROG", title = "Outras receitas do produtor -<br/>Agricultura Patronal", id = "LOCALIZACAO", palette="Purples",
...
* Proporção de estabelecimentos dos produtores beneficiários do Pronamp:

$$\tiny
prop.Estabelecimentos.MP = \frac {Numero.Estabelecimentos.MP} {Estabelecimentos.Total}
$$

* Proporção de estabelecimentos não familiares (patronais):

$$\tiny
prop.Estabelecimentos.AP = \frac {Numero.Estabelecimentos.AP} {Estabelecimentos.Total}
$$
Onde:<br/>
_Numero.Estabelecimentos.AF_ = Estabelecimentos da agricultura familiar da microrregião;<br/>
_Numero.Estabelecimentos.MP_ = Estabelecimentos do produtor beneficiado pelo Pronamp da microrregião;<br/>
_Numero.Estabelecimentos.AP_ = Estabelecimentos que não são de agricultura familiar (patronais);<br/>
...
NEstab <- 
  ...
  labs(x = "", y = "", title = "Proporção de estabelecimentos", subtitle = "Participação por tipologia no número de estabelecimentos") +
  scale_color_brewer(palette = "Dark2", direction=-1, guide = "none") +
  theme_light() +
  theme(legend.position = 'none',
        plot.title = element_text(hjust = 0.5),
...
* Apesar de uma média de participação baixa quando comparado aos da AP, no geral, é importante apontara maior porcentagens de estabelecimentos da AP e de beneficiários do Pronamp, além da consequente menor porcentagem da AF nessa região.
...
SHP_Micro_join[!is.na(SHP_Micro_join$prop_NEstab),] %>%
  ...
  filter(TIPOLOGIA == "GP") %>%
tm_shape(bbox = bbox_new) +
  tm_fill(col = "prop_NEstab", title = "Proporção de estabelecimentos -<br/>Agricultura Patronal", id = "LOCALIZACAO", palette="Purples",
          legend.format=list(fun=function(x) paste0(formatC(x*100, digits=0, format="f"), "%"))) +
  tm_borders(lwd = 0.2) +
  #tm_facets(by = "TIPOLOGIA", nrow = 1, sync = TRUE, free.coords = TRUE) +
  tm_view(view.legend.position = c("right", "bottom")) +
...
* Proporção de área dos estabelecimentos do produtor beneficiário do Pronamp:

$$\tiny
prop.AreaEstabelecimento.MP = \frac {AreaEstabelecimento.MP} {AreaEstabelecimento.Total}
$$

* Proporção de área dos estabelecimentos não familiares (patronais):

$$\tiny
prop.AreaEstabelecimento.AP = \frac {AreaEstabelecimento.AP} {AreaEstabelecimento.Total}
$$
Onde:<br/>
_AreaEstabelecimento.AF_ = Área em hectares (ha) dos estabelecimentos da agricultura familiar da microrregião;<br/>
_AreaEstabelecimento.MP_ = Área em hectares (ha) dos estabelecimentos do produtor beneficiado pelo Pronamp da microrregião;<br/>
_AreaEstabelecimento.AP_ = Área em hectares (ha) dos estabelecimentos que não são de agricultura familiar (patronais);<br/>
...
AREA <- 
  ...
  labs(x = "", y = "", title = "Proporção de área total dos estabelecimentos", subtitle = "Participação por tipologia na área total dos estabelecimentos") +
  scale_color_brewer(palette = "Dark2", direction=-1, guide = "none") +
  theme_light() +
  theme(legend.position = 'none',
        plot.title = element_text(hjust = 0.5),
...
* Apesar dos estabelecimentos de beneficiários do Pronamp apresentarem altos valores de participação na área total dos estabelecimentos, a categoria não aparenta ter uma significante concentração espacial em uma região específica;
* A agricultura não familiar (patronal) é a tipologia que apresenta valores mais altos, indicando uma proporção de área maior para esse tipo de estabelecimento;
  - Entretando, é necessário notar na diferença entre a AP e a AF nas diversas regiões: No Centro-Oeste a diferença entre essas duas é muito grande, com quase a totalidade das áreas das microrregiões sendo ocupadas por estabelecimentos não familiares. Já nas regiões Nordeste e Norte, apesar da porcentagem dos estabelecimentos da AP ainda ser maior, a diferença para os da AF é muito menor, estando mais próximo um equilíbrio entre a área ocupada por essas duas classes.
	<br/>
	<br/>
...
SHP_Micro_join[!is.na(SHP_Micro_join$prop_AREA),] %>%
  ...
  filter(TIPOLOGIA == "GP") %>%
tm_shape(bbox = bbox_new) +
  tm_fill(col = "prop_AREA", title = "Proporção de área total<br/>dos estabelecimentos -<br/>Agricultura Patronal", id = "LOCALIZACAO", palette="Purples",
          legend.format=list(fun=function(x) paste0(formatC(x*100, digits=0, format="f"), "%")),
          style = "fixed", breaks = c(0, 0.3, 0.5, 0.7, 1)) +
  tm_borders(lwd = 0.2) +
  #tm_facets(by = "TIPOLOGIA", nrow = 1, sync = TRUE, free.coords = TRUE) +
  tm_view(view.legend.position = c("right", "bottom")) +
  tm_shape(SHP_Estados) +
...
* Proporção de pessoal ocupado do produtor beneficiário do Pronamp:

$$\tiny
prop.PessoalOcupado.MP = \frac {Numero.PessoalOcupado.MP} {Numero.PessoalOcupado.T} 
$$

* Proporção de pessoal ocupado nos estabelecimentos não familiares (patronais):

$$\tiny
prop.PessoalOcupado.AP = \frac {Numero.PessoalOcupado.AP} {Numero.PessoalOcupado.T} 
$$
Onde:<br/>
_Numero.PessoalOcupado.AF_ = Número de pessoal ocupado nos estabelecimentos da agricultura familiar da microrregião;<br/>
_Numero.PessoalOcupado.MP_ = Número de pessoal ocupado nos estabelecimentos do produtor beneficiado pelo Pronamp da microrregião;<br/>
_Numero.PessoalOcupado.AP _ =  Número de pessoal ocupado nos estabelecimentos que não são de agricultura familiar (patronais);<br/>
...
OCU <- 
  ...
  labs(x = "", y = "", title = "Proporção de pessoal ocupado nos estabelecimentos", subtitle = "Participação por tipologia no número de ocupados nos estabelecimentos") +
  scale_color_brewer(palette = "Dark2", direction=-1, guide = "none") +
  theme_light() +
  theme(legend.position = 'none',
        plot.title = element_text(hjust = 0.5),
...
* Em nas regiões, exceto Centro-Oeste e Sudeste, a maior parte de pessoas ocupadas nos estabelecimentos agropecuários é da agricultura familiar, chegando a média de 78% no Norte do país e 72% no Nordeste;
* Na agricultura não familiar (patronal), são os estabelecimentos do Centro-Oeste e do Sudeste que mais empregam;
* Embora possua média no geral inferior tanto a patronal quanto a familiar, os estabelecimentos do produtor atendido pelo Pronamp possuem a menor variância de ocupados dentre as microrregiões.
...
SHP_Micro_join[!is.na(SHP_Micro_join$prop_OCU),] %>%
  ...
  filter(TIPOLOGIA == "GP") %>%
tm_shape(bbox = bbox_new) +
  tm_fill(col = "prop_OCU", title = "Proporção de pessoal ocupado<br/>nos estabelecimentos -<br/>Agricultura Patronal", id = "LOCALIZACAO", palette="Purples",
          legend.format=list(fun=function(x) paste0(formatC(x*100, digits=0, format="f"), "%")),
          style = "quantile",
          n = 4) +
  tm_borders(lwd = 0.2) +
...
* Proporção do valor da produção do produtor beneficiário do Pronamp:

$$\tiny
prop.ValorProducao.MP = \frac {ValorProducao.MP} {ValorProducao.T} 
$$

* Proporção do valor da produção do produtor não familiar (patronal):

$$\tiny
prop.ValorProducao.AP = \frac {ValorProducao.AP} {ValorProducao.T} 
$$
Onde:<br/>
_ValorProducao.AF_ = Valor da produção (mil reais) nos estabelecimentos da agricultura familiar da microrregião;<br/>
_ValorProducao.MP_ = Valor da produção (mil reais) nos estabelecimentos do produtor beneficiado pelo Pronamp da microrregião;<br/>
_ValorProducao.AP _ =  Valor da produção (mil reais) nos estabelecimentos que não são de agricultura familiar (patronais);<br/>
...
VP <- 
  ...
  labs(x = "", y = "", title = "Proporção de valor da produção dos estabelecimentos", subtitle = "Participação por tipologia no valor da produção dos estabelecimentos") +
  scale_color_brewer(palette = "Dark2", direction=-1, guide = "none") +
  theme_light() +
  theme(legend.position = 'none',
        plot.title = element_text(hjust = 0.5),
...
* A proporção dos valores do VP nos três tipos de estabelecimentos varia muito de acordo com a região;
  + Apesar da AF possuir um maior número de estabelecimentos em todas as regiões, no Centro-Oeste o valor dela é muito inferior ao da AP, seguido por uma diferença menor, mas ainda muito grande, no Sudeste e, depois, Sul;
  + Já na região Norte e Nordeste a diferença entre a AF e a AP é praticamente inexistente, indicando que nesses locais, em média, o valor da produção dos estabelecimentos familiares e patronais foi muito similar;

* A média da proporção do VP nos estabelecimentos dos beneficiários do Pronamp é relativamente estável em todas as regiões, apresentando uma menor variância entre os valores, ou seja, os estabelecimentos médios possuem uma geração de valor da produção bastante próximos entre si;
  + Esse fenômeno ocorre apenas para os estabelecimentos médios, nos da AF e patronais os resultados das microrregiões são mais dispersos, contando com valores muito altos, mas também muito baixos dentro de cada região, exceto no Centro-Oeste, onde a variância interna é baixa.
...
SHP_Micro_join[!is.na(SHP_Micro_join$prop_VP),] %>%
  ...
  filter(TIPOLOGIA == "GP") %>%
tm_shape(bbox = bbox_new) +
  tm_fill(col = "prop_VP", title = "Valor da produção<br/>nos estabelecimentos -<br/>Agricultura Patronal", id = "LOCALIZACAO", palette="Purples",
          legend.format=list(fun=function(x) paste0(formatC(x*100, digits=0, format="f"), "%"))) +
  tm_borders(lwd = 0.2) +
  #tm_facets(by = "TIPOLOGIA", nrow = 1, sync = TRUE, free.coords = TRUE) +
  tm_view(view.legend.position = c("right", "bottom")) +
...
#### VP por área total dos estabelecimentos {.tabset .tabset-fade}
...
* Valor da produção por hectare do produtor beneficiário do Pronamp:

$$\tiny
prop.ValorProducaoArea.MP = \frac {ValorProducao.MP} {AreaEstabelecimento.MP} 
$$

* Valor da produção por hectare do produtor não familiar (patronal):

$$\tiny
prop.ValorProducaoArea.AP = \frac {ValorProducao.AP} {AreaEstabelecimento.AP} 
$$
Onde:<br/>
_ValorProducao.AF_ = Valor da produção (mil reais) nos estabelecimentos da agricultura familiar da microrregião;<br/>
_ValorProducao.MP_ = Valor da produção (mil reais) nos estabelecimentos do produtor beneficiado pelo Pronamp da microrregião;<br/>
...
```{r COD_Preparação dos dados, message=FALSE, warning=FALSE, include=FALSE}
...
microrregioes_VPha_pivot$TIPOLOGIA <- factor(microrregioes_VPha_pivot$TIPOLOGIA, levels = c("TT", "GP", "MP", "AF"))
...
VP_AREA <-
  ...
  labs(x = "", y = "VP (por mil) / ha ", title = "Valor da produção por hectare nas microrregiões") +
  ...
  scale_x_discrete(labels=c("Todos os estab.", "Agricultura não familiar", "Pronamp", "Agricultura Familiar")) +
  coord_flip()
...
* A média do valor da produção por hectare é muito maior nas regiões Sul e Sudeste;

* Em todas as regiões, exceto no Centro-Oeste, o valor da produção por hectare da AF é maior do que da AP (com pouca diferença); 

* Algumas poucas microrregiões apresentam valores muito altos de VP/ha em todas as regiões.
...
#### VP por área dos estabelecimentos não destinada a preservação permanente ou reserva legal {.tabset .tabset-fade}

Para o cálculo da razão entre o valor da produção e a área do estabelecimento (em ha) não destinada a preservação permanente ou reserva legal, ou seja, a área útil do estabelecimento, foi utilizado:

* Valor da produção por hectare útil da agricultura familiar:

$$\tiny
prop.ValorProducaoAreaUtil.AF = \frac {ValorProducao.AF} {AreaUtilEstabelecimento.AF} 
$$

* Valor da produção por hectare útil do produtor beneficiário do Pronamp:

$$\tiny
prop.ValorProducaoAreaUtil.MP = \frac {ValorProducao.MP} {AreaUtilEstabelecimento.MP} 
$$

* Valor da produção por hectare útil do produtor não familiar (patronal):

$$\tiny
prop.ValorProducaoAreaUtil.AP = \frac {ValorProducao.AP} {AreaUtilEstabelecimento.AP} 
$$
Onde:<br/>
_ValorProducao.AF_ = Valor da produção (mil reais) nos estabelecimentos da agricultura familiar da microrregião;<br/>
_ValorProducao.MP_ = Valor da produção (mil reais) nos estabelecimentos do produtor beneficiado pelo Pronamp da microrregião;<br/>
_ValorProducao.AP _ =  Valor da produção (mil reais) nos estabelecimentos que não são de agricultura familiar (patronais);<br/>
_ValorProducao.T_ = Valor da produção (mil reais) nos estabelecimentos agropecuários na microrregião. <br/>
_AreaUtilEstabelecimento.AF_ = Área em hectares (ha) dos estabelecimentos da agricultura familiar da microrregião;<br/>
_AreaUtilEstabelecimento.MP_ = Área em hectares (ha) dos estabelecimentos do produtor beneficiado pelo Pronamp da microrregião;<br/>
_AreaUtilEstabelecimento.AP_ = Área em hectares (ha) dos estabelecimentos que não são de agricultura familiar (patronais);<br/>

```{r COD_Preparação dos dados2, message=FALSE, warning=FALSE, include=FALSE}
microrregioes2 <- microrregioes %>% 
  left_join(., TABELA9, by = "COD")

microrregioes_VPhaUTIL <- microrregioes2 %>%
...
```{r graf ESTABELECIMENTOS - Valor da produção por área util, message=FALSE, warning=FALSE, include=FALSE}
microrregioes_VPhaUTIL_pivot$TIPOLOGIA <- factor(microrregioes_VPhaUTIL_pivot$TIPOLOGIA, levels = c("TT", "GP", "MP", "AF"))
...
```{r grafPLOT ESTABELECIMENTOS - Valor da produção por área util, echo=FALSE, message=FALSE, warning=FALSE, out.width='100%'}
gg_VP_AREAUTIL <- ggplotly(VP_AREAUTIL, tooltip = c("label", "y")) %>%
  layout(margin = margens,
    title = list(text = paste0('Valor da produção por hectare útil nas microrregiões')))
...
```{r mapa ESTABELECIMENTOS - Valor da produção por área util, message=FALSE, warning=FALSE, include=FALSE}
## AF
tm_AF_VPdivAREAUTIL <- 
microrregioes_VPhaUTIL_shape[!is.na(microrregioes_VPhaUTIL_shape$VPdivAREA),] %>%
  filter(VPdivAREA >= 0) %>%
  filter(LOCALIZACAO != "Fernando de Noronha (PE)") %>%
  filter(TIPOLOGIA == "AF") %>%
  tm_shape(bbox = bbox_new) +
...
```{r mapaPLOT ESTABELECIMENTOS - Valor da produção por área util, echo=FALSE, message=FALSE, warning=FALSE}
tmap_arrange(tm_AF_VPdivAREAUTIL, tm_GP_VPdivAREAUTIL, tm_MP_VPdivAREAUTIL, tm_TT_VPdivAREAUTIL,
             sync = TRUE, 
             nrow = 2)
```
...
## Alguns apontamentos {.tabset .tabset-fade}

* Apesar de corrigido o erro quanto ao cálculo dos estabelecimentos patronais, tornando-os sinônimo de não familiar, algumas questões sobre essa categoria permanecem:
  + Nesse método de considerar a agricultura patronal, me parece que muitos estabelecimentos distintos entre si estão sendo colocados juntos em uma mesma categoria; a maioria das variáveis mostra uma dispersão grande nos dados dessa tipologia, levantando o questionamento de ser necessário ou não trabalhar em cima desta classe de estabelecimento para distrincha-la em duas categorias distintas.
...
* Agricultura familiar, Pronamp e Patronal funcionam de maneiras bastante distintas, tanto espacialmente quanto estatisticamente
	+ Enquanto que para a AF os valores proporcionais costumam ser mais elevados na região Norte, Nordeste e Sul, os valores para o Pronamp parecem apresentar uma menor correlação espacial (valores similares estão mais dispersos por toda a extensão do território), ao passo que o Centro-Oeste (e, em alguns casos, o Sudeste e Sul) se destaca como região para altos valores proporcionais em praticamente todos os recortes de estabelecimentos patronais; 
	+ Os valores do Pronamp possuem menor variância entre si, ou seja, estão próximos entre si, sem grandes 'outliers' em nenhuma região, diferentemente dos valores da AF ou da AP, que por vezes possuem microrregiões com valores muito altos e muito baixos;
	+ A partir dessa exploração, não recomendo a junção das classes de AF com do Pronamp ou do Pronamp com não familiar (patronal) para o prosseguimento do trabalho.

* Sobre a predominância de tipologias (em área):
	+ Para a agricultura familiar, apenas uma microrregião, a do Rio Negro no Amazonas, possui predominância acima de 80% em área; na região Sul, no norte do Rio Grande do Sul e oeste do Paraná, a AF possui predominância de 40% a 60% em área dos estabelecimentos; outra região significativa para essa tipologia é a nordeste, que possui uma concentração de microrregiões com valores acima de 40%;
	+ Com exceção de algumas áreas do norte, nordeste, leste de Minas Gerais, e porções dos estados do Rio Grande do Sul e Paraná, os estabelecimentos não familiares possuem larga predominância na proporção de áreas. Ainda assim, mesmo considerando a baixa quantidade de áreas da AF onde mais de 70% da área é ocupada por AP, em alguns desses locais a receita desse tipo de estabelecimento ainda consegue ser considerável, exceto no Centro-Oeste, onde a AF parece possuir muito menos força em todas as variáveis;
	+ Os estabelecimentos do Pronamp dividem espaço com os não familiares no sul do Rio Grande do Sul, Santa Catarina e Paraná, mas ganham uma predominância de acima de 80% em área em toda a faixa próxima ao litoral no estado de São Paulo e Rio de Janeiro; em Minas Gerais, enquanto os estabelecimentos patronais dividem espaço mais na porção oeste, sentido o Centro-Oeste, os médios prevalecem na faixa central do estado e perdem força a leste, dividindo ali a região com a agricultura familiar; na região Norte, as microrregiões mais próximas da divisa com o Mato Grosso são as que predominam os estabelecimentos do médio produtor, além disso, ao extremo norte, nas microrregiões em Roraima e no Amapá, a proporção desses estabelecimentos é superior a 80%;

* Com a correção da categoria de estabelecimentos patronais, a hipótese de que adicionar a variável de valor da produção auxiliaria em resultados mais precisos sobre a agricultura familiar para a clusterização ainda é confirmada, apesar de possuir menos discrepâncias com a variável de valor total das receitas. Com diferenças presentes, mas mais sutis entre as variáveis de VP e valor das receitas, a variável ainda pode auxiliar no melhor detalhamento e diferenciação dos estabelecimentos da AF, especialmente na utilização da variável de VP/ha;
	+ No valor da produção por hectare (VP/ha) os destaques são distintos dos destaqueas do VP ou do valor das receitas: no geral, a região Sul e Sudeste possui um VP/ha maior que as demais regiões; o noroeste do estado do Amazonas também se destaca aqui nos estabelecimentos da AF e do produtor do Pronamp.

<br/>

__* Sobre próximos passos:__
<br/>
1. Acredito que, para iniciar, seria interessante refazer a clusterização realizada pelo Pedro Cavalcante apenas com as variáveis da Agricultura Familiar, com a adição do valor da produção e valor da produção por hectare;

2. Em seguida, possuo três propostas de caminhos distintos para o trabalho:
  a) Após essa primeira aproximação, apesar da caracterização da agricultura familiar ser o interesse principal, penso que poderia ser relevante realizar a clusterização utilizando também os dados dos estabelecimentos do Pronamp (médios) e os patronais; A ideia aqui seria fazer três clusterizações distintas, uma com variáveis da AF, outra com do Pronamp e outra com dos patronais e, a partir desses três resultados, fazer uma última clusterização que teria como objetivo caracterizar não apenas os três tipos de estabelecimentos agropecuários, como também entender as relações/coexistências entre eles;
  
  b) Uma outra alternativa, que depende do direcionamento que se deseja dar ao trabalho, é realizar apenas a clusterização inicial com os dados da agricultura familiar e, para refinar a análise, adicionar as variáveis socioeconômicas de interesse advindas de outras fontes de dados (que seja possível agregar para o recorte da microrregião, como dados do Censo Demográfico). Refazendo a clusterização com um conjunto de novas variáveis provavelmente seria necessário adicionar a etapa de análise de componentes principais (para reduzir o conjunto das variáveis em grupos) e a partir disso realizar a clusterização;
  
  c) Por fim, uma última alternativa seria prosseguir na análise exploratória das variáveis antes da realização da clusterização, mas acrescentando dados do Censo Agro de 2006. Esse caminho vai depender da intenção de direcionamento da pesquisa, se é caminhar para uma caracterização atual dos estabelecimentos/produtores ou para uma comparação das mudanças ocorridas em um período de 10 anos; Uma vez que já possuímos a clusterização de ambos os anos realizada anteriormente, a sugestão aqui seria acrescentar algumas variáveis na análise de cada ano para obter resultados distintos e mais direcionados na comparação dos dois anos.
...
+ [Tabela 6882 - Número de estabelecimentos agropecuários com área e Área dos estabelecimentos agropecuários, por tipologia, utilização das terras, classe de idade do produtor, cor ou raça do produtor e grupos de área total](https://sidra.ibge.gov.br/tabela/6882)
