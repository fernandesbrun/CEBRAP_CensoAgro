...
kbl(tabelaRECEITAS, digits = 2, caption = "Origem da renda dos estabelecimentos por tipologia", align = "clccccc") %>%
  ...
  footnote(symbol = "Os valores dos grandes produtores são calculados a partir dos totais subtraídos dos valores da agricultura familiar e de beneficiados pelo Pronamp, por essa razão, nem sempre os valores obtidos representam exatidão.",
          footnote_as_chunk = T)
```
...
* Proporção da renda de produção no médio produtor rural:

$$\tiny
prop1.ReceitaProducao.MP = \frac {ReceitaProducao.MP} {Receita.MP} 
$$

* Proporção da renda de produção no grande produtor rural:

$$\tiny
prop1.ReceitaProducao.GP = \frac {ReceitaProducao.GP} {Receita.GP} 
$$
Onde:<br/>
...
SHP_Micro_join[!is.na(SHP_Micro_join$prop3_REC_PROD),] %>%
  ...
  filter(TIPOLOGIA == "GP") %>%
tm_shape(bbox = bbox_new) +
  tm_fill(col = "prop3_REC_PROD", title = "Receita da produção -<br/>Outros (patronal)", id = "LOCALIZACAO", palette="Purples",
          legend.format=list(fun=function(x) paste0(formatC(x*100, digits=0, format="f"), "%"))) +
  tm_borders(lwd = 0.2) +
  #tm_facets(by = "TIPOLOGIA", nrow = 1, sync = TRUE, free.coords = TRUE) +
  tm_view(view.legend.position = c("right", "bottom")) +
...
* Proporção das rendas de produção advindas do médio produtor rural:

$$\tiny
prop2.ReceitaProducao.MP = \frac {ReceitaProducao.MP} {ReceitaProducao.T} 
$$

* Proporção das rendas de produção advindas do grande produtor rural:

$$\tiny
prop2.ReceitaProducao.GP = \frac {ReceitaProducao.GP} {ReceitaProducao.T} 
$$
Onde:<br/>
_ReceitaProducao.AF_ =  Rendas da produção (mil reais) nos estabelecimentos da agricultura familiar da microrregião;<br/>
_ReceitaProducao.MP_ = Rendas da produção (mil reais) nos estabelecimentos do médio produtor rural da microrregião;<br/>
...
* Proporção de outras receitas do estabelecimento no médio produtor rural:

$$\tiny
prop1.ReceitaEstabelecimento.MP = \frac {ReceitaEstabelecimentoOutras.MP} {Receita.MP} 
$$

* Proporção de outras receitas do estabelecimento no grande produtor rural:

$$\tiny
prop1.ReceitaEstabelecimento.GP = \frac {ReceitaEstabelecimentoOutras.GP} {Receita.GP} 
$$
Onde:<br/>
...
_ReceitaEstabelecimentoOutras.MP_ = Outras rendas do estabelecimento (mil reais) nos estabelecimentos do médio produtor rural da microrregião;<br/>
_ReceitaEstabelecimentoOutras.GP _ = Outras rendas do estabelecimento (mil reais) nos estabelecimentos que não são nem agricultura familiar, nem médio produtor da microrregião (patronal/grande produtor rural);<br/>
...
* Embora o valor advindo de outras receitas no estabelecimento seja baixo na composição da renda da maior parte dos estabelecimentos das microrregiões, algumas poucas localidades demonstram valores mais elevados que a média, podendo indicar de forma bastante específica que aquela microrregião explora melhor outras atividades possíveis para a obtenção de renda. 
	- Uma localidade importante a ser observada é a região Sudeste e Nordeste, onde a dispersão (variância) nessa categoria de renda é maior para os estabelecimentos patronais.
	- Apesar dos estabelecimentos patronais apresentarem, no geral, uma média ligeiramente maior que os demais quando se trata de rendimentos de outras atividades no estabelecimento para a composição da renda, são o médio produtor (no Centro-Oeste e Sudeste) e a agricultura familiar (no Nordeste, Norte e Sul) que utilizam uma porção maior da receita total das atividades que não são produção; ou seja, apesar de estar nos estabelecimentos da AF e da MP uma parcela maior desse tipo de rendimento, no total das receitas de cada um a categoria já não é tão significativa;
...
SHP_Micro_join[!is.na(SHP_Micro_join$prop3_REC_OUTR),] %>%
  ...
  filter(TIPOLOGIA == "GP") %>%
tm_shape(bbox = bbox_new) +
  tm_fill(col = "prop3_REC_OUTR", title = "Outras receitas<br/>do estabelecimento -<br/>Outros (patronal)", id = "LOCALIZACAO", palette="Purples",
          legend.format=list(fun=function(x) paste0(formatC(x*100, digits=0, format="f"), "%"))) +
  tm_borders(lwd = 0.2) +
  #tm_facets(by = "TIPOLOGIA", nrow = 1, sync = TRUE, free.coords = TRUE) +
  tm_view(view.legend.position = c("right", "bottom")) +
...
$$\tiny
prop2.ReceitaEstabelecimento.GP = \frac {ReceitaEstabelecimento.GP} {ReceitaEstabelecimento.T} 
$$
Onde:<br/>
...
SHP_Micro_join[!is.na(SHP_Micro_join$prop_REC_OUTR),] %>%
  ...
  filter(TIPOLOGIA == "GP") %>%
tm_shape(bbox = bbox_new) +
  tm_fill(col = "prop_REC_OUTR", title = "Outras receitas<br/>do estabelecimento -<br/>Outros (patronal)", id = "LOCALIZACAO", palette="Purples",
          legend.format=list(fun=function(x) paste0(formatC(x*100, digits=0, format="f"), "%"))) +
  tm_borders(lwd = 0.2) +
  #tm_facets(by = "TIPOLOGIA", nrow = 1, sync = TRUE, free.coords = TRUE) +
  tm_view(view.legend.position = c("right", "bottom")) +
...
$$\tiny
prop.ReceitaFora.GP = \frac {ReceitaFora.GP} {Receita.GP} 
$$
Onde:<br/>
...
* O tipo de estabelecimento com resultados mais dispersos é o do grande produtor (patronal), especialmente no Nordeste.<br/>
<br/>
<br/>
...
SHP_Micro_join[!is.na(SHP_Micro_join$prop3_REC_FORA),] %>%
  ...
  filter(TIPOLOGIA == "GP") %>%
tm_shape(bbox = bbox_new) +
  tm_fill(col = "prop3_REC_FORA", title = "Receitas fora<br/>do estabelecimento -<br/>Outros (patronal)", id = "LOCALIZACAO", palette="Purples",
          legend.format=list(fun=function(x) paste0(formatC(x*100, digits=0, format="f"), "%")),
          style = "kmeans") +
  tm_borders(lwd = 0.2) +
  #tm_facets(by = "TIPOLOGIA", nrow = 1, sync = TRUE, free.coords = TRUE) +
...
* Proporção das rendas de atividades fora do estabelecimento no médio produtor rural:

$$\tiny
prop2.ReceitaFora.MP = \frac {ReceitaFora.MP} {ReceitaFora.T} 
$$

* Proporção das rendas de atividades fora do estabelecimento no grande produtor rural:

$$\tiny
prop2.ReceitaFora.GP = \frac {ReceitaFora.GP} {ReceitaFora.T} 
$$
Onde:<br/>
_ReceitaFora.AF_ =  rendas de Rendas de atividades fora do estabelecimento (mil reais) nos estabelecimentos da agricultura familiar da microrregião;<br/>
_ReceitaFora.MP_ = rendas de Rendas de atividades fora do estabelecimento (mil reais) nos estabelecimentos do médio produtor rural da microrregião;<br/>
...
* Proporção de outras receitas do produtor para o médio produtor rural:

$$\tiny
prop1.ReceitaProgPoli.MP = \frac {ReceitaProgPoli.AF} {Receita.MP} 
$$

* Proporção das rendas de atividades fora do estabelecimento para o grande produtor rural:

$$\tiny
prop1.ReceitaProgPoli.GP = \frac {ReceitaProgPoli.AF} {Receita.GP} 
$$
Onde:<br/>
...
SHP_Micro_join[!is.na(SHP_Micro_join$prop3_REC_PROG),] %>%
  ...
  filter(TIPOLOGIA == "GP") %>%
tm_shape(bbox = bbox_new) +
  tm_fill(col = "prop3_REC_PROG", title = "Receitas de<br/>programas e políticas -<br/>Outros (patronal)", id = "LOCALIZACAO", palette="Purples",
          legend.format=list(fun=function(x) paste0(formatC(x*100, digits=0, format="f"), "%")),
...
* Proporção das rendas de programas e políticas no médio produtor rural:

$$\tiny
prop2.ReceitaProgPoli.MP = \frac {ReceitaProgPoli.MP} {ReceitaProgPoli.T} 
$$

* Proporção das rendas de programas e políticas no grande produtor rural:

$$\tiny
prop2.ReceitaProgPoli.GP = \frac {ReceitaProgPoli.GP} {ReceitaProgPoli.T} 
$$
Onde:<br/>
_ReceitaProgPoli.AF_ =  Rendas de programas e políticas (mil reais) do produtor nos estabelecimentos da agricultura familiar da microrregião;<br/>
_ReceitaProgPoli.MP_ = Rendas de programas e políticas (mil reais) do produtor nos estabelecimentos do médio produtor rural da microrregião;<br/>
...
SHP_Micro_join[!is.na(SHP_Micro_join$prop_REC_PROG),] %>%
  ...
  filter(TIPOLOGIA == "GP") %>%
tm_shape(bbox = bbox_new) +
  tm_fill(col = "prop_REC_PROG", title = "Outras receitas do produtor -<br/>Outros (patronal)", id = "LOCALIZACAO", palette="Purples",
...
* Apesar dos estabelecimentos do médio produtor apresentar altos valores de participação na área total dos estabelecimentos, a categoria não aparenta ter uma significante concentração espacial em uma região específica;
	+ A agricultura familiar e a patronal, apesar de apresentarem médias de participações por região com valores não tão discrepantes entre si, a primeira concentra sua alta participação mais no sertão nordestino, na região do Rio Negro no Amazonas e no norte do Rio Grande do Sul, enquanto que a segunda tem maior participação no Centro-Oeste e no interior dos estados de São Paulo e Paraná
	<br/>
	<br/>
...
tm_shape(bbox = bbox_new) +
  tm_fill(col = "prop_AREA", title = "Proporção de área total<br/>dos estabelecimentos -<br/>Outros (patronal)", id = "LOCALIZACAO", palette="Purples",
          legend.format=list(fun=function(x) paste0(formatC(x*100, digits=0, format="f"), "%")),
          style = "fixed", breaks = c(0, 0.15, 0.3, 0.5, 0.92)) +
  tm_borders(lwd = 0.2) +
  #tm_facets(by = "TIPOLOGIA", nrow = 1, sync = TRUE, free.coords = TRUE) +
  tm_view(view.legend.position = c("right", "bottom")) +
  tm_shape(SHP_Estados) +
...
* Em três regiões, Nordeste, Norte e Sul, a média do VP na agricultura familiar foi superior a dos outros tipos de estabelecimento;
  + Nas duas regiões em que a AF não é a com maior proporção de valor da produção, o estabelecimento patronal é predominante: com larga diferença no Centro-Oeste (65% dos patronais e 10% da AF) e menor vantagem no Sudeste (40% dos patronais e 30% da AF);
 
* A média da proporção do VP nos estabelecimentos do médio produtor é relativamente estável em todas as regiões, apresentando uma menor variância entre os valores, ou seja, os estabelecimentos médios possuem uma geração de valor da produção bastante próximos entre si;
  + Esse fenômeno ocorre apenas para os estabelecimentos médios, nos da AF e patronais os resultados das microrregiões são mais dispersos, contando com valores muito altos, mas também muito baixos dentro de cada região.
...
* Em todas as regiões, o valor da produção por hectare dos estabelecimentos patronais é maior do que dos outros tipos de estabelecimento;
  + Um destaque maior é dado para a região nordeste, onde o valor da produção toma mais distância dos valores da AF e do médio produtor. Importante ressaltar que são poucas as microrregiões nordestinas que possuem o VP/ha alto nos estabelecimentos patronais, o que eleva a média desse tipo na região são localidades separadas, sem um contínuo espacial.
...
* Os dados patronais não são muito confiáveis para análise: somando a AF e a MP nas microrregiões, em diversos casos o resultado é superior ao total, tornando o valor das grandes propriedades negativos. A hipótese é de que existe um erro no dado original, visto que, em teoria, deveria ser impossível que a soma de duas tipologias que não acontecem concomitantemente fosse superior ao total.
  - Qual seria a solução então para trabalhar com dados patronais? É possível descartar esses casos onde o valor é claramente errado, mas enfraquece um pouco a metodologia manter apenas os dados que "parecem" corretos, sem necessariamente uma confirmação que estão corretos ou não.
...
* Agricultura familiar, Pronamp e Patronal funcionam de maneiras bastante distintas, tanto espacialmente quanto estatisticamente
	+ Enquanto que para a AF os valores proporcionais costumam ser mais elevados na região Norte, Nordeste e Sul, os valores para o Pronamp (médios produtores) parecem apresentar uma menor correlação espacial (valores similares estão mais dispersos por toda a extensão do território), ao passo que o Centro-Oeste se destaca como região para altos valores proporcionais em todos os recortes de estabelecimentos patronais
	+ A partir dessa exploração, não recomendaria a junção das classes de AF com MP ou de MP com patronal para o prosseguimento do trabalho
	
* Sobre a predominância de tipologias (em área):
	+ Os grandes estabelecimentos predominam em algumas porções do Rio Grande do Sul (a leste do estado), no interior do estado de São Paulo, no Centro-Oeste (exceto Mato Grosso do Sul) e na divisa entre os estados do Tocantins, Maranhão, Piauí e Bahia.
	+ Os estabelecimentos do Pronamp dividem espaço com os patronais no sul do Rio Grande do Sul, Santa Catarina e Paraná, mas ganham uma predominância de acima de 80% em área em toda a faixa próxima ao litoral no estado de São Paulo e Rio de Janeiro; em Minas Gerais, enquanto os estabelecimentos patronais dividem espaço mais na porção oeste, sentido o Centro-Oeste, os médios prevalecem na faixa central do estado e perdem força a leste, dividindo ali a região com a agricultura familiar; na região Norte, as microrregiões mais próximas da divisa com o Mato Grosso são as que predominam os estabelecimentos do médio produtor, além disso, ao extremo norte, nas microrregiões em Roraima e no Amapá, a proporção desses estabelecimentos é superior a 80%;
...
* A hipótese de que adicionar a variável de valor da produção auxiliaria em resultados mais precisos sobre a agricultura familiar para a clusterização é confirmada na comparação entre a distribuição espacial desta, da receita da produção e do valor da produção/ha. Resultando em áreas de enfoque com diferentes regiões, essa variável auxiliará no melhor detalhamento e diferenciação dos estabelecimentos da AF nas diferentes regiões;
	+ Diferentemente da variável de participação da receita da produção na receita do estabelecimento, que forma uma grande faixa com valores acima de 50% do Sul ao Norte (exceto Norteste e estado do Amazonas, Acre e Roraima), ou a variável de participação da AF no total das receitas, que forma uma faixa da região norte a nordeste, com focos específicos na sul, mostra que no Nordeste, apesar da renda da produção ser baixa quando comparada às demais regiões, o valor da produção não é; além disso, a região Sul não aparece mais de forma tão dominante na AF;
	+ No valor da produção por hectare (VP/ha) os destaques são ainda mais diferentes: no geral, a região Sul e Sudeste possui um VP/ha maior que as demais regiões; o noroeste do estado do Amazonas também se destaca aqui nos estabelecimentos da AF e do médio produtor.
...
2. Em seguida, possuo três propostas de caminhos distintos para o trabalho:
  a) Após essa primeira aproximação, apesar da caracterização da agricultura familiar ser o interesse principal, penso que poderia ser relevante realizar a clusterização utilizando também os dados dos estabelecimentos do Pronamp (médios) e, talvez, os patronais (devido o problema apresentado no primeiro tópico, existe uma confiabilidade menor na utilização dos dados patronais, mas podemos pensar se valeria a pena fazer uma limpeza nos valores impossíveis, como valores de área negativos, e utilizar essa categoria mesmo assim); A ideia aqui seria fazer três clusterizações distintas, uma com variáveis da AF, outra com do Pronamp e outra com dos patronais e, a partir desses três resultados, fazer uma última clusterização que teria como objetivo caracterizar não apenas os três tipos de estabelecimentos agropecuários, como também entender as relações/coexistências entre eles;
